...
def _rebuild_sellitems_seg(text, hs, he, block, tail_comma, newline):
    ...
    return text[:hs] + "".join(block) + text[he:]
...
def _equip_del_entry(text, key, eqo, nm, arr_open, arr_close):
    ...
    if p >= eqo and text[p] == ",":      # 非首条: 前导逗号一并删
        k0 = p
    return text[:k0] + text[arr_close + 1:]
    ...
    for nm, target in pairs.items():
        ...
        if target > cur:
            ...
            for _ in range(need_del):
                ...
                _lo, _lc = sp[-2]
                vo, vc = sp[-1]
                text = text[: _lc + 1] + text[vc + 1:]   # 去掉前导分隔与最后一个实例
            changed[nm] = target
        # target == cur: 不变
    return text, changed
...
def shop_add_equips(text, key, adds):
    ...
    for nm, cnt in merged.items():
        ...
        entries = []
        for kd, vs, _ve, _vt in _entry_values(seg, rel):
            if seg[vs:vs + 1] == "[":
                entries.append((_es3_unescape_name(kd), vs, _matching_end(seg, vs)))
        idx = next((i for i, (n, _a, _b) in enumerate(entries) if n == nm), None)
        if idx is not None:
            # 同名已上架: 往其数组里追加副本(定位最后实例对象的闭花括号后插入)
            _n, va, _vacl = entries[idx]
            last = va
            for _k3, v3, _e3, _t3 in _entry_values(seg, va):
                if seg[v3:v3 + 1] == "{":
                    last = _matching_end(seg, v3)
            ap = ""
            for i in range(cnt):
    ...
    return text[:hs] + seg + text[he:], result
...
class MainWindow(QMainWindow):
    ...
    def _build_shop_tab(self):
        ...
        rv.addWidget(_g)
        # —— 批量折扣/刷新日(可一键应用到 选中 或 全部 NPC; 目标值可自定义并自动记忆) ——
        ...
        self.btn_npc_sel.setToolTip("把上方批量折扣/刷新日应用到左侧选中的 NPC(可 Ctrl 多选; 无选中=当前选中)")
        self.btn_npc_all = QPushButton("应用到全部 NPC")
        self.btn_npc_all.setToolTip("把上方批量折扣/刷新日一键应用到全部商店 NPC(含没有该字段的店自动补写)")
        ...
        # —— 卖品操作行: 添加卖品/添加装备(原有) + 删除选中卖品/选中改数量(多选) ——
        _arow = QHBoxLayout()
        ...
        self.shop_batch_spin.setToolTip("商店卖品「一键改数量」/装备「改件数」的目标值; 修改自动记忆")
        self.shop_batch_spin.valueChanged.connect(self._on_shop_batch_value_changed)
        self.btn_shop_item_del = QPushButton("删除选中卖品")
        self.btn_shop_item_del.setToolTip("把右侧卖品表选中的道具整件下架(可 Ctrl/拖动多选); 需确认")
        self.btn_shop_item_del.clicked.connect(self._on_shop_item_del)
        self.btn_shop_item_set = QPushButton("选中改数量")
        self.btn_shop_item_set.setToolTip("把右侧卖品表选中道具的数量改成左侧数字框的值")
        self.btn_shop_item_set.clicked.connect(self._on_shop_item_set)
        _irow = QHBoxLayout()
        _irow.addWidget(self.btn_shop_item_del)
        _irow.addWidget(self.btn_shop_item_set)
        _irow.addWidget(QLabel("目标:"))
        _irow.addWidget(self.shop_batch_spin)
        _irow.addStretch(1)
        rv.addLayout(_irow)
        # —— 已上架装备(SellEquips): 表 + 删除整种/改件数(可多选) ——
        _eqg = QGroupBox("已上架装备 (SellEquips)")
        _ev = QVBoxLayout(_eqg)
        ...
        self.equip_table.setToolTip("该 NPC 已上架装备(同名可多件); 可多选后删除或改件数(件数目标=上方数字框, 0=删除整种)")
        _ev.addWidget(self.equip_table, 1)
        ...
        self.btn_equip_set.setToolTip("把选中装备的件数(同名副本数)改成上方数字框的值; 0=删除整种")
        self.btn_equip_set.clicked.connect(self._on_shop_equip_set)
        _erow = QHBoxLayout()
        _erow.addWidget(self.btn_equip_del)
        _erow.addWidget(self.btn_equip_set)
        _erow.addWidget(QLabel("目标件数: 上方数字框"))
        _erow.addStretch(1)
        _ev.addLayout(_erow)
        rv.addWidget(_eqg)
        rv_box = QWidget()
    ...
    def _on_shop_add_equip(self):
        ...
        def _work():
            nt, added = shop_add_equips(text, key, [(n, cnt) for n in names])
            if added:
                _get_struct_index(nt)   # 后台线程预热新文本结构索引
            return nt, added
