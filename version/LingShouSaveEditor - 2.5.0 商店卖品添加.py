...
版本: v2.5.0 (2026-09-05)

新增(v2.5.0): 「商店NPC」页签——枚举 saveUtilData.value._ShopData 的 NPC/商店(abName), 支持
            中文/拼音/首字搜索; 选中后查看其 SellItems 现有卖品, 点「添加卖品…」复用「新增道具」的
            搜索/多选对话框添加道具(数量默认 999, 与已有同名自动跳过), 随「保存」写回存档。
            道具字典段解析引擎参数化(key="SellItems"), 支持段尾有/无逗号两种收尾。
"""
import json
import os
import re
...
class MainWindow(QMainWindow):
    ...
    def _reload_table(self):
        ...
        if self._filter_map is None:
            ...
            for r, real in enumerate(self._filter_map):
                self._fill_item_row(r, real, self._items[real])
        self._loading = False
        if self._filter_map is not None:
            self.count_label.setText("道具总数: %d · 显示: %d" % (len(self._items), shown))
        else:
    ...
    def _build_shop_tab(self):
        ...
        tip = QLabel("NPC/商店卖的道具: 左侧选 NPC/商店(abName, 支持中文/拼音/首字搜索), "
                     "右侧为其 SellItems 现有卖品, 可「添加卖品…」从道具名库搜索/多选(数量默认 999, 与已有同名自动跳过)。")
        tip.setWordWrap(True)
        ...
        rv.addWidget(self.btn_add_sell)
        rv_box = QWidget()
        rv_box.setLayout(rv)
    ...
    def _refresh_shop_tab(self):
        """按 self._text 重建商店缓存与 NPC 列表(读档后/初始化时调用)。"""
        text = self._text or ""
        self._shop_cache = {}
        self._shop_key = None
    ...
    def _filter_shop_list(self):
        ...
        for r in range(self.shop_list.count()):
            ...
            if not q:
                ...
                it.setHidden(not filter_item_names([it.text()], q, pm, mode=0))
    ...
    def _show_shop_items(self):
        """右侧表显示当前 NPC 的 SellItems 卖品(名称/数量)。"""
        key = self._shop_key
        self.shop_items.setRowCount(0)
        ...
        self.shop_title.setText("NPC: %s · 卖品 %d 件" % (ab, len(items)))
    ...
    def _open_big(self, path, text, bom, record):
        ...
        def work():
            try:
                ...
                if si < 0:
                    res["err"] = "未找到 ItemHad 段, 无法解析该内容"
                    return
                items, indent, ci = parse_itemhad(lines, si)
                if ci is None:
                    res["err"] = "ItemHad 段解析失败, 请确认内容完整"
                    return
                res["items"] = items
                res["indent"] = indent
                res["model"] = char_model(text)
                res["lib"] = merge_item_names_from_text(text)
            except Exception as e:  # noqa: BLE001
                res["err"] = str(e)
            finally:
                done.append(1)
        ...
        if "err" in res:
            QMessageBox.warning(self, "提示", res["err"])
            return False
        self._text = text
        self._bom = bom
        self._items = res["items"]
        self._indent = res["indent"]
        self._file_path = path
        self._lib_new, self._lib_total = res["lib"]
        self._reload_table()
        self._apply_char_model(res["model"])
        self._refresh_shop_tab()
        self._mark_clean()
        self.src_label.setText("源: %s" % path)
        self.statusBar().showMessage(
...
9. <b>商店NPC卖的道具(v2.5.0)</b>: 主区「商店NPC」页签——左侧列出存档里的 NPC/商店(取自
   saveUtilData.value._ShopData 的 abName), 支持 <b>中文 / 拼音 / 首字</b> 搜索; 选中某 NPC 后右侧显示
   其 SellItems 现有卖品(名称/数量); 点「<b>添加卖品…</b>」从 道具名.json 搜索(中文/拼音/首字/数字)
   或<b>多选</b>一次添加多个(数量默认 <b>999</b>, 可在框内改), 与已有同名道具自动跳过, 不重复;
   点「保存/另存为/复制结果」时随存档一起写回。
