...
版本: v2.4.0 (2026-09-04)

新增(v2.4.0): 从 MapJumpTable.bytes 提取地图跳转点 → 程序目录 地图跳转点.json(_scaffold/
            build_map_jump_points.py 生成); 「地图标注点(_ActiveMap)」新增「从地图跳转点批量添加…」
            ——搜索(中文/拼音)/多选/全选一次添加多个跳转点, 默认只列未标注的, 已有重复自动跳过。
"""
import json
import os
import re
