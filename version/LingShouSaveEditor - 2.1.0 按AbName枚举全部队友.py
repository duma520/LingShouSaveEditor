...
def friend_records(text):
    ...
    遍历 saveFriendData.value 的每一个直接对象字段(容器), 把其中“直接含 _AbName 字符串字段”的子对象
    都视为一名可改队友(游戏后续新增队友时, 无论加到哪个容器都会自动被收进来)。
    ...
    for group, gs, _ge, _gt in _entry_values(text, val):
        ...
        for key, vs, _ve, _vt in _entry_values(text, gs):
            ...
            out.append((group, key, nm or key))
    return out
...
class MainWindow(QMainWindow):
    ...
    def _refresh_char_tabs(self):
        ...
        text = self._text or ""
        records = friend_records(text) if text else []
        has_player = _locate_container(text, SCOPE_PLAYER) is not None
        has_friends = bool(records)
        ...
        if has_friends:
            for group, key, disp in records:
                suffix = ""
                if group != "AddFriends":
                    suffix = "(%s)" % FRIEND_GROUP_LABELS.get(group, group)
                title = "队友·%s%s" % (disp, suffix)
                self.tabs.addTab(self._build_friend_page(group, key, disp), title)
    ...
    # --- 队友页 ---
    def _build_friend_page(self, group, key, disp):
...
7. <b>多角色分页</b>: 打开含角色数据的存档后, 主区顶部会多出「主角」与每个「队友·名字」页签
   ...
   页内「地图标注点(_ActiveMap)」可输入地点名新增/选中删除。
