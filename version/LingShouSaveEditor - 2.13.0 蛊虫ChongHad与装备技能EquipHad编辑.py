...
新增(v2.13.0 蛊虫+装备技能): **新增「蛊虫(ChongHad)」与「装备技能(EquipHad)」两个页签**。
            ...
            一键 9999 或自定义值, 可一键「补齐全部状态(5 种)」或「写入全部技能库(370 项)」;
            ...
            不是按 内存所有道具.json 的 skill 段全量 370 条); 新增蛊虫的图标按名字元素推断
...
_SCAN_CACHE = [None, None]   # [text对象, (poss, chars)] 只缓存最近一份文本, 防大档多份滞留
...
def _matching_end(text, start):
    ...
    基于结构索引线性扫描, 大文本远快于逐 token finditer。
    """
    opener = text[start]
    closer = "}" if opener == "{" else "]"
    poss, chars = _get_struct_index(text)
    np = len(poss)
...
def _entry_values(text, open_i):
    ...
    基于结构索引: 先用同型配平找容器闭(同 _matching_end 语义), 再在闭区间内做全括号计数切 0 层逗号
    (数组/对象都占层, 屏蔽子容器内逗号), 与原 finditer 语义一致; 大文本远快。
    """
    opener = text[open_i]
    closer = "}" if opener == "{" else "]"
    poss, chars = _get_struct_index(text)
    np = len(poss)
    # 1) 同型配平找容器自己的闭(与 _matching_end 一致)
    i = bisect_left(poss, open_i)
    depth0 = 0
    close_i = -1
    while i < np:
        c = chars[i]
    ...
    if close_i < 0:
        close_i = len(text) - 1
    # 2) 闭区间内: 全括号计数(任何 { [ 都 +1, } ] 都 -1), 逗号在 0 层即容器顶层分隔
...
def _apply_subs(text, subs):
    ...
    if cached and old_idx is not None:
        try:
            ...
            _SCAN_CACHE[0] = nt
            _SCAN_CACHE[1] = (new_poss, new_chars)
...
#     (如 "提供真气":[\r\n\t\t\t\t\t\t\t7\r\n\t\t\t\t\t\t]), 技能名取自全局技能库
#     (技能名.json = 内存所有道具.json 的 dataset/skill/*, 370 条)。
...
CHONG_PROP_DEFAULT = 999          # Power/Agility/PhysicalPower 默认值
CHONG_SKILL_DEFAULT = 9999        # _Skill 各状态默认值
CHONG_PREF_DEFAULT = 9999         # _Preference 各状态默认值
CHONG_FOSTER_MAX_DEFAULT = 5      # 新增蛊虫 _FosterMax 默认
...
def load_skill_categories(path=None):
    """读 技能名.json(结构同 道具分类.json); 缺失/损坏返回 None。"""
    return _load_cat_json(path or get_skill_categories_path())


def load_skill_names(path=None):
    """全部技能名(370 条)扁平列表; 缺失返回 []。"""
...
class AddItemDialog(QDialog):
    ...
    def __init__(self, parent=None, existing=None, default_name="", default_count=9999,
        ...
        if self._use_cat:
            ...
            self._all_names = list(dict.fromkeys(
    ...
    def _update_stat(self):
        ...
        if self._use_cat:
            ...
            self.stat_label.setText("%s名库 %d · 当前勾选分类含 %d · 已选 %d"
                                    % (self._noun, total, shown, sel))
        else:
            self.stat_label.setText("%s名库 %d · 个人已有 %d · 缺少 %d · 已选 %d"
                                    % (self._noun, total, total - miss, miss, sel))
...
class AddChongDialog(AddItemDialog):
    ...
    - 三项属性 Power(力道)/Agility(灵气)/PhysicalPower(体魄) 默认 999, 可自定义;
      新增的每只蛊虫 _TotalProperty 一律写 0。
    - _Skill(状态累积值) / _Preference(状态倍率) 可一键写入「全部状态」
      (中毒/流血/着火/点穴/冰冻 5 种) 或「全部技能库」(技能名.json 的 370 项), 默认 9999、可自定义。
    """
    ...
    def _build_extra(self, root):
        ...
        self.chk_skill_all = QCheckBox("改写入全部技能库(370 项)")
        self.chk_skill_all.setChecked(bool(d.get("fill_all_skill", False)))
        self.chk_skill_all.setToolTip(
            "不勾=只写上面 5 种蛊虫可用状态; 勾上=把 技能名.json 里全部 370 条技能名都写进 _Skill。\n"
...
class MainWindow(QMainWindow):
    # 固定页签: 0=道具 / 1=商店NPC / 2=蛊虫 / 3=装备技能; 角色页签从 4 开始(读档后动态增删)
    CHAR_TAB_BASE = 4
    ...
    def _build_ui(self):
        ...
        # 装备技能页签(固定 index=3): 读档后由 _refresh_equip_tab 填充 EquipHad 实例
        self.tabs.addTab(self._build_equip_tab(), "装备技能")
    ...
    def _build_chong_tab(self):
        ...
        self.btn_chong_add.setToolTip(
            ...
            "新增时可设默认 力道/灵气/体魄(默认 999) 与 _Skill/_Preference 的默认值(默认 9999)，\n"
            "并可选「写入全部状态」或「写入全部技能库(370 项)」。")
    ...
    def _chong_build_attr_page(self):
        ...
        form.addRow(QLabel("培养上限 _FosterMax"), self.chong_fostermax_spin)
        v.addLayout(form)
    ...
    def _chong_build_dict_page(self, field, title):
        ...
        tip = QLabel(
            ...
            "「写入全部技能库」= 把 技能名.json 的全部 370 条技能名都写入（存档会变大，游戏可能忽略不认识的技能名）。"
            % title)
        tip.setWordWrap(True)
        tip.setStyleSheet("color:#888;")
        v.addWidget(tip)
        ...
        btn_fill.clicked.connect(lambda _=False, f=field: self._chong_set_all(f, self._chong_dict_spins[f].value(),
                                                                             list(CHONG_SKILLS)))
        btn_lib = QPushButton("本只：写入全部技能库(370 项)")
        btn_lib.setToolTip("把 技能名.json 的全部技能名都写入（仅调试用；存档会明显变大）")
    ...
    def _refresh_chong_tab(self, model=None):
        ...
        self._chong_model = model if model is not None else (
            chong_model(self._text) if self._text else [])
    ...
    # ---------------- 装备技能(EquipHad 实例的 _SkillLv/_ItemSkillLv)页(v2.13.0) ----------------
    def _build_equip_tab(self):
        """装备技能页: 左=装备名列表, 右上=该装备的实例(等级/耐久), 右下=实例的技能字典。"""
        page = QWidget()
        lay = QVBoxLayout(page)
        ...
        note = QLabel(
            "装备属性 / 技能(EquipHad)：每个装备实例可改 _Lv(等级) / _Durable(耐久)，"
    ...
    def _refresh_equip_tab(self, model=None):
        """按当前 self._text 重建装备列表(模型只保留“装备名 + 件数”; 实例区间选中时按名字重定位)。"""
        if getattr(self, "equip_name_list", None) is None:
            return
        full = model if model is not None else (
            equip_skill_model(self._text) if self._text else [])
        # v2.13.0: 只缓存名字与件数 —— 实例的绝对下标每次用时按名字重定位,
        # 这样文本被改动(甚至被蛊虫页改动)后也不会读到错位的字段。
        self._equip_model = [(nm, len(spans) if isinstance(spans, list) else int(spans))
                             for nm, spans in full]
        cur = self.equip_name_list.currentItem()
        cur_txt = cur.text() if cur is not None else None
        self._equip_loading = True
        try:
            self.equip_name_list.clear()
            for nm, cnt in self._equip_model:
                self.equip_name_list.addItem("%s  (%d 件)" % (nm, cnt))
            self.equip_count_label.setText("装备: %d 种" % len(self._equip_model))
            if cur_txt:
                for r in range(self.equip_name_list.count()):
                    if self.equip_name_list.item(r).text() == cur_txt:
                        self.equip_name_list.setCurrentRow(r)
                        break
        finally:
            self._equip_loading = False
    ...
    def _filter_equip_list(self):
        """按搜索框过滤装备名列表(只隐藏, 不影响数据)。"""
        if getattr(self, "equip_name_list", None) is None:
            return
        q = (self.equip_search.text() or "").strip()
        pmap = load_pinyin_map() if q else None
        keep = set(filter_item_names([nm for nm, _s in self._equip_model], q, pmap)) if q else None
        for r in range(self.equip_name_list.count()):
            it = self.equip_name_list.item(r)
            nm = self._equip_model[r][0] if r < len(self._equip_model) else it.text()
            it.setHidden(bool(keep is not None and nm not in keep))

    def _equip_cur_name(self):
        """当前选中的装备名; 未选返回 None。"""
        it = self.equip_name_list.currentItem()
        if it is None:
            return None
        for nm, _c in self._equip_model:
            if it.text().startswith(nm + "  ("):
                return nm
        return None

    def _on_equip_name_selected(self):
        """装备列表选中变化: 按名字重新定位实例区间并重填实例表。"""
        ...
        try:
            ...
            self._equip_loading = False
        self._equip_inst_open = spans[0][0] if spans else None
    ...
    def _refresh_shop_tab(self, cache=None):
        ...
        为 None 时同步计算(仅小档/无 _ShopData 用)。之后商店读取一律 fast 实时定位, 不依赖缓存偏移。
        """
        text = self._text or ""
        recs = cache
        if recs is None:
    ...
    def _refresh_char_tabs(self):
        ...
        if model["player"] is None and not model["friends"]:
            # 保留 道具(0)+商店NPC(1)+蛊虫(2)+装备技能(3), 移除之后的旧角色页
...
10. <b>蛊虫(ChongHad)页(v2.13.0)</b>: 主区「蛊虫」页签——左侧列出存档 ChongHad 里的蛊虫(显示名 · 数据集名),
    ...
    · 「<b>＋ 添加蛊虫…</b>」: 候选来自 程序目录/蛊虫名.json(由 _scaffold/build_chong_names.py 从
      ...
      新增时可设 三项属性默认值(默认 999)与 _Skill/_Preference 目标值(默认 9999, 可自定义), 并可选
...
11. <b>装备技能(EquipHad)页(v2.13.0)</b>: 主区「装备技能」页签——左侧是主角 EquipHad 里的装备名(可搜索),
    右上列出该装备的实例(# / 等级 _Lv / 耐久 _Durable, 可直接双击改), 右下分两页
    「装备技能 _SkillLv」与「词条技能 _ItemSkillLv」——存档里它们是「技能名 → [等级数组]」
    (如 "提供真气":[7])。可 表格内逐条改等级 / 「＋ 添加技能…」(从 技能名.json 搜索多选, 统一等级) /
    「选中行 → 目标等级」/ 「－ 删除选中」; 字段不存在时写回会自动补一个。改完点「保存/另存为/复制结果」写回。
...
- <b>技能名.json</b>(build_skill_names.py): 蛊虫 _Skill/_Preference 与装备技能的技能名库
  (内存所有道具.json 的 dataset/skill 全量 370 条)。<br>
  打包 exe 时这些 json 会随程序目录一起带上。
