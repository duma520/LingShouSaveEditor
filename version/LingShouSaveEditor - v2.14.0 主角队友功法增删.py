...
def kangfu_add(text, container_open, adds, data=None):
    """把 [(功法名, 等级)] 写进该角色的对应功法段(段按名字在库里的分支自动判定)。

    返回 (新文本, 实际新增 [(段标题, 名)], 已学过跳过 [名], 无法判定/该角色缺该段 [名])。
    ...
    for seg in KANGFU_SEG_ORDER:
        ...
        l2 = l0 + unit * 2
        l3 = l0 + unit * 3
        have = {_kangfu_norm(n) for n, _ks, _vs, _vc in _kangfu_entries(seg_txt, rel)}
        for nm, lv in items:
            if _kangfu_norm(nm) in have:
                skipped.append(nm)
                continue
            esc = _es3_escape_name(nm)
            entry = ('"' + esc + '":{' + nl + _kangfu_fields_text(seg, esc, lv, l2, l3, nl)
                     + l1 + '}')
            entries = _kangfu_entries(seg_txt, rel)
            if entries:
                j = entries[-1][3]          # 最后一个条目的值闭括号
                seg_txt = seg_txt[:j + 1] + "," + entry + seg_txt[j + 1:]
            else:
                # 段原本为空(形如 {  + 换行缩进 + }): 首条目内联在 `{` 之后
                seg_txt = seg_txt[:rel + 1] + entry + nl + l0 + seg_txt[seg_close - hs:]
            have.add(_kangfu_norm(nm))
...
def char_model(text):
    ...
    返回 {
      'player': {scalars, dicts, open, kangfu} | None,
      'friends': [ {group, key, name, scalars, dicts, open, kangfu} ... ],
      'maps': [地名...],
...
class KangFuDialog(AddItemDialog):
    ...
    def __init__(self, parent=None, existing=None, title="添加功法",
        ...
        super().__init__(parent, existing=existing, default_count=default_lv,
                         ...
                         title=title, noun="功法",
                         count_label="等级 _Lv(新增功法的等级, 默认 %d):" % KANGFU_DEFAULT_LV)
...
class MainWindow(QMainWindow):
    ...
    def _refresh_equip_tab(self, model=None):
        ...
        try:
            ...
            for nm, k, dup in self._equip_rows:
                self.equip_name_list.addItem("%s #%d" % (nm, k + 1) if dup else nm)
    ...
    def _add_kangfu_section(self, scope, brief=None):
        ...
        btn_add.setToolTip(
            ...
            "可搜索(中文/拼音/首字母)/多选/全选一次添加多门; 已经学过的灰显跳过。")
        ...
        row.addWidget(cnt)
        v.addLayout(row)
        self._kangfu_ui[key] = {"list": lst, "count": cnt}
    ...
    def _fill_kangfu_list(self, key=None, brief=None):
        ...
        for k in keys:
            ...
            for seg, title, nm in items:
                ...
                it.setToolTip("所在段: %s(队友为 _%s)" % (seg, seg))
                lst.addItem(it)
            ui["brief"] = items
            cnt = ui.get("count")
            if cnt is not None:
    ...
    def _on_kangfu_add(self, scope):
        ...
        if co is None:
            ...
            return
        existing = {nm for _s, _t, nm, _ks, _vs, _vc in kangfu_records(self._text, co)}
        dlg = KangFuDialog(self, existing=existing)
        if not dlg.exec():
            return
        names = list(dict.fromkeys(dlg.chosen_names()))
        if not names:
            return
        lv = int(dlg.count_spin.value())
        text = self._text
        ...
        def _work():
            ...
            if added:
                _get_struct_index(nt)   # 后台线程预热新文本结构索引
            return nt, added, skipped, unknown

        ok, r = self._run_bg(_work, "正在添加功法…请稍候")
        if not ok:
            self.statusBar().showMessage("添加功法失败: %s" % r, 4000)
            return
        nt, added, skipped, unknown = r
        if added:
            self._text = nt
            self._mark_dirty()
            self._fill_kangfu_list(tuple(scope))
    ...
    def _build_friend_page(self, group, key, disp, scalars=None, dicts=None, kangfu=None):
        ...
        lay.addWidget(self._add_kangfu_section(scope, kangfu or []))
        return page
...
12. <b>功法(内功/武功/绝技/轻功) —— 主角/队友页(v2.14.0)</b>: <b>主角页与每个队友页都新增「功法」分组</b>,
    ...
    · <b>怎么判断一门功法该放哪段</b>= 看它在游戏数据集里属哪一支(即 内存所有道具.json 的
      ...
      (实测: 主角 KangFuHad 里 211 条名字 100%% 落在 dataset/kangfu/kangfu/*; KangFuSkillHad 85 条全在
      uniqueskill/jue/*; KangFuQingHad 54 条全在 uniqueskill/qing/*; InKangFuHad 66 条全在 inkangfu/*。)<br>
    ...
    · 对话框下方「<b>等级 _Lv</b>」= 写进这些新功法条目的等级(默认 1)。<b>已经会的功法会灰显跳过</b>,
    ...
    · 改动只写在内存里, 与其它功能一样点工具栏「<b>保存 / 另存为 / 复制结果</b>」才写回存档。
