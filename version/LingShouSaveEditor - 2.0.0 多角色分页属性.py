...
def _num_token(text, val_start):
    """若 val_start 处是数字则返回 (数字记号原串, 记号结束下标); 否则 None。"""
    m = re.match(r"[+-]?[0-9]*\.?[0-9]+(?:[eE][+-]?[0-9]+)?", text[val_start:])
    if not m:
        return None
    return text[val_start:val_start + m.end()], val_start + m.end()
...
def set_flat_dict_all(text, scope, field, new_num):
    ...
    for key_disp, vs, _ve, _vt in _entry_values(text, c):
        if key_disp == field and text[vs:vs + 1] == "{":
            ...
            for s, e, v in sorted(subs, key=lambda x: x[0], reverse=True):
...
class MainWindow(QMainWindow):
    ...
    def _refresh_char_tabs(self):
        ...
        text = self._text or ""
        has_player = _locate_container(text, SCOPE_PLAYER) is not None
        has_friends = _locate_container(text, SCOPE_FRIENDS) is not None
        if not (has_player or has_friends):
            empty = QWidget()
            ...
            return
        if has_player:
            self.tabs.addTab(self._build_player_page(), "主角")
        if has_friends:
            for k in friend_keys(text):
                title = "队友·%s" % self._friend_display(k)
                self.tabs.addTab(self._build_friend_page(k), title)
    ...
    def _apply_scalar(self, scope, field, value):
        ...
        if self._text is None:
            return
        nt = set_scalar(self._text, scope, field, value)
    ...
    def _apply_char_defaults(self, scope, defaults):
        ...
        nt = self._text
        changed = False
        for key, _raw in list_scalar_fields(nt, scope):
            d = defaults.get(key)
            if d is None:
                continue
            n2 = set_scalar(nt, scope, key, d)
            if n2 is not nt:
                nt = n2
                changed = True
        if changed:
    ...
    # --- 标量区 ---
    def _add_scalar_section(self, title, scope, defaults, labels):
        """把 scope 容器内全部数字字段排成 名称+当前值+默认按钮 行; 顶部放“一键全部按默认”。"""
        ...
        self._building_char_page = True
        try:
            for key, raw in list_scalar_fields(self._text or "", scope):
    ...
    # --- 编号/名字字典区 ---
    def _add_dict_section(self, title, scope, field, default, key_labels=None, note=""):
        """把某角色的 {编号/名字: 数值} 字典显示成行, 提供「全部→默认值」按钮。"""
        entries = iter_flat_dict(self._text or "", scope, field)
        group = QGroupBox(title)
        v = QVBoxLayout(group)
        if note:
            n = QLabel(note)
    ...
    # --- 地图标注点(_ActiveMap)区 ---
    def _add_map_section(self):
        entries = get_str_array(self._text or "", SCOPE_MAP, "_ActiveMap")
        ...
        self._map_list.addItems(entries)
        v.addWidget(self._map_list)
        row = QHBoxLayout()
        self._map_edit = QLineEdit()
        self._map_edit.setPlaceholderText("输入要新增的地点名…")
    ...
    def _build_player_page(self):
        ...
        lay.addWidget(head)
        lay.addWidget(self._add_scalar_section("基础属性", SCOPE_PLAYER, PLAYER_DEFAULTS, PLAYER_LABELS))
        ...
        lay.addWidget(self._add_map_section())
        return page

    # --- 队友页 ---
    def _build_friend_page(self, key):
        scope = SCOPE_FRIENDS + [key]
        page = self._char_page_scroll()
        lay = self._char_inner_lay
        nm = get_str_field(self._text or "", scope, "_Name")
        head = QLabel("队友：%s(%s)  · 修改后点「保存/另存为/复制结果」写回存档。" % (nm or key, key))
        head.setWordWrap(True)
        head.setStyleSheet("font-weight:bold;")
        lay.addWidget(head)
        lay.addWidget(self._add_scalar_section("队友属性", scope, FRIEND_DEFAULTS, FRIEND_LABELS))
        lay.addWidget(self._add_dict_section(
...
7. <b>多角色分页</b>: 打开含角色数据的存档后, 主区顶部会多出「主角」与每个「队友·名字」页签
   (v2.0.0 新增): 每页可逐个直接改 等级/经验/属性/血量/特性点/已用点等数值——带「默认」按钮的
   字段点按钮即一键设默认(也可点「一键全部按默认」); 主角页另可整组改 <b>武学经验 KongFuTypeLv</b>
   (默认一键 999999)、<b>生活技能 LifeExp</b>(999999)、<b>六维 SixProCurrent</b>(999)、
   <b>好感度 FriendLoveNum</b>(999, 前面是好友名字); 每个队友页也有其 六维(999) 与
   武学经验 _KongFuTypeLv(999999)。页内「地图标注点(_ActiveMap)」可输入地点名新增/选中删除。
