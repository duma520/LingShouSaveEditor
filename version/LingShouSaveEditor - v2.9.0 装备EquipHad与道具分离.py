...
def _player_equip_entry_text(l1, l2, l3, l4, name_esc, lv, nl):
    """一个新的 EquipHad 条目(单实例):  "名字":[\n<实例>\nL1]\n  (不含外层逗号)。"""
    return ('"' + name_esc + '":[' + nl +
            l2 + "{" + nl +
            _player_equip_fields_text(l3, l4, name_esc, lv, nl) +
            l2 + "}" + nl +
            l1 + "]")


def player_add_equips(text, names):
    """把若干装备名追加到主角 EquipHad(已在段的同名跳过; 每名 1 实例; 名字前导数字=等级)。

    返回 (新文本, 实际新增名列表)。仅在 EquipHad 字典段内做局部改写(经 _apply_subs 维护索引缓存);
    找不到 EquipHad 段返回 (原文本, [])。
    """
    names = list(dict.fromkeys((n or "").strip() for n in (names or []) if (n or "").strip()))
    if not names:
        return text, []
    o = player_equip_open(text)
    if o is None:
    ...
    nl = "\r\n" if "\r\n" in body else "\n"
    hs = text.rfind("\n", 0, o) + 1
    l0 = re.match(r"[ \t]*", text[hs:o]).group(0)
    unit = "\t" if "\t" in l0 else (l0[:1] if l0 else "\t")
    l1 = l0 + unit
    l2 = l0 + unit * 2
    l3 = l0 + unit * 3
    l4 = l0 + unit * 4
    have = {nm for nm, _a, _c in _sell_equip_entries_in(text, o)}
    added = []
    parts = []
    for nm in names:
        if nm in have:
        ...
        mnum = re.match(r"(\d+)", nm)
        lv = int(mnum.group(1)) if mnum else 1
        parts.append(_player_equip_entry_text(l1, l2, l3, l4, _es3_escape_name(nm), lv, nl))
        have.add(nm)
        added.append(nm)
    if not added:
        return text, []
    ...
    tail = body[len(core):]
    if core:
        # 已有条目: 在最后一条的数组闭合 `]` 后紧跟 `,新名`(与游戏多条目样式一致), 再补回原尾部换行/缩进
        newbody = core + "".join("," + p for p in parts) + tail
    else:
        # EquipHad 原本为空: 首条内联在 `{` 后(同商店 SellEquips 空字典加首键的写法)
        newbody = "".join(("," + p if i else p) for i, p in enumerate(parts)) + tail
    return _apply_subs(text, [(o + 1, close, newbody)]), added
    ...
    def __init__(self, parent=None, existing=None, default_name="", default_count=9999,
        ...
        if self._use_cat:
            ...
            self._all_names = list(dict.fromkeys(
                _filter_names_by_cats(load_item_names(), self._allow, self._cat_data)))
        ...
        # 数量(仅添加道具/商店装备等需要; 主角装备 EquipHad 每件 1 个, 由调用方隐藏此行)
        cnt = QHBoxLayout()
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.btn_add_player_equip.setToolTip(
            ...
            "弹窗只显示 equip 分类; 已在 EquipHad 的装备名灰显防重(每件 1 个, 前导数字=等级);\n"
            "点「保存/另存为/复制结果」随存档写回。")
    ...
    def _on_add_player_equip(self):
        ...
        equip 不进 ItemHad(道具页新增已排除 equip); 弹窗只列 equip 分类、已有装备名灰显防重;
        每件 1 个实例(名字前导数字=等级), 写入 savePlayerData.value.EquipHad 段,
        ...
        if self._text is None:
            ...
            return
        existing = {n for n, _c in player_equip_records(self._text)}
        dlg = AddItemDialog(self, existing=existing, default_count=1,
                            allow_codes=EQUIP_ADD_CATS, show_count=False)
        ...
        def _work():
            nt, added = player_add_equips(text, names)
            if added:
                _get_struct_index(nt)   # 后台线程预热新文本结构索引
            return nt, added
        ...
        if added:
            ...
            self._mark_dirty()
        dup = len(names) - len(added)
        msg = "已为主角添加装备 %d 件(写入 EquipHad, 每件 1 个)" % len(added)
        if dup:
            msg += " · %d 个已在 EquipHad 自动跳过" % dup
        self.statusBar().showMessage(msg, 4000)
