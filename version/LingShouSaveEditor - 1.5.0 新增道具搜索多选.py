...
def merge_item_names_from_text(text, path=None):
    ...
    merged = sorted(cur | extracted)   # 与库中已有名比对后合并去重
...
class AddItemDialog(QDialog):
    ...
    # ---------------- 内部 ----------------
    def _filtered(self):
        return filter_item_names(self._all_names, self.search.text(),
    ...
    def _update_stat(self):
        vis = self.item_list.count()
        sel = len(self.item_list.selectedItems())
        self.stat_label.setText("候选 %d / 共 %d · 已选 %d" % (vis, len(self._all_names), sel))
...
class MainWindow(QMainWindow):
    ...
    def _on_header_clicked(self, col):
        """点击表头排序: 列1=道具名称(按名), 列2=数量(按值); 同列再点一次反向。

        直接对 self._items 排序后整表重建, 表格行序恒等于列表序,
        因此排序后双击改名/改数量、删除选中仍按原行号映射到道具, 不影响修改。
        """
        if col not in (1, 2):
            return
        hdr = self.table.horizontalHeader()
        cur = hdr.sortIndicatorSection()
        order = hdr.sortIndicatorOrder()
        if cur == col:
            order = (Qt.SortOrder.DescendingOrder if order == Qt.SortOrder.AscendingOrder
        ...
        self.statusBar().showMessage(
            "已按%s排序 %s" % ("名称" if col == 1 else "数量",
                               "↑升序" if not reverse else "↓降序"), 2000)
...
2. 表格会枚举出 ItemHad 里全部道具的 <b>名称</b> 和 <b>数量</b>, 可直接双击单元格修改;
   点击表头「道具名称 / 数量」可按名称或数量排序(再点一次反向), 排序不影响修改。
