...
新增(v2.14.2 功法等级持久记忆): **「添加功法…」对话框里的「等级 _Lv」改过就记住**(用户反馈:
            ...
            `_save_settings_now` 同步写入; 取消对话框则不记。对话框标签也改为
...
class KangFuDialog(AddItemDialog):
    ...
    def __init__(self, parent=None, existing=None, title="添加功法",
                 default_lv=KANGFU_DEFAULT_LV, dressed=None):
        tips = {}
        ...
        super().__init__(parent, existing=existing, default_count=default_lv,
                         ...
                         title=title, noun="功法",
                         count_label="等级 _Lv(新增功法的等级; 改过会记住, 下次打开还是它):",
                         existing_tips=tips)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # 「添加功法…」的等级 _Lv 记忆(v2.14.2): 改过就记住, 下次打开还是这个值, 不用每次重改
        try:
    ...
    def _add_kangfu_section(self, scope, brief=None, dressed=None):
        ...
        btn_add.setToolTip(
            ...
            "对话框里的「等级 _Lv」改过会记住(存 设置.json), 下次打开还是这个值。")
    ...
    def _on_kangfu_add(self, scope):
        ...
        if co is None:
            ...
            return
        inner, dressed = kangfu_dressed(self._text, co)
        ...
        dlg = KangFuDialog(self, existing=existing, dressed=dressed,
                           default_lv=self._kangfu_lv)      # 等级记住上次改过的值(v2.14.2)
        if not dlg.exec():
            return
        names = list(dict.fromkeys(dlg.chosen_names()))
        if not names:
        ...
        # 记住本次等级(下次打开对话框就是它), 与其它设置一样防抖写进 设置.json
        lv = max(0, min(lv, MAX_ITEM_COUNT))
        if lv != self._kangfu_lv:
            self._kangfu_lv = lv
            self._autosave_settings()
        text = self._text
        ...
        if added:
            ...
            if ui is not None:
                ui["dressed"] = None      # 文本已变 → 填充时按新文本重算已装备信息
...
12. <b>功法(内功/武功/绝技/轻功) —— 主角/队友页(v2.14.0)</b>: <b>主角页与每个队友页都新增「功法」分组</b>,
    ...
    · 对话框下方「<b>等级 _Lv</b>」= 写进这些新功法条目的等级(默认 1); <b>改过会记住</b>
      (存 设置.json 的 kangfu_lv, 下次打开对话框还是这个值, 不用每次重改)。<b>已经会的功法会灰显跳过</b>,
