...
版本: v2.16.0 (2026-09-17)

新增(v2.16.0 一键清除 + 道具黑名单): 用户要求 ①「功法黑名单增加一键清除主角/队友已经学习了的黑名单功法」、
            ②「增加道具黑名单(主要是把黑名单功法的同名秘籍一并列入), 添加道具时不会添加道具黑名单的道具,
            并能一键清除主角包裹里被列入黑名单的道具」。实现:
            ① **黑名单逻辑抽成通用层**: `_load_blacklist_file`/`_save_blacklist_file`/`_blacklist_keys`
            两个名单共用; 新增 道具黑名单 纯函数 `get_item_blacklist_path`(程序目录/道具黑名单.json)/
            `load_item_blacklist`/`save_item_blacklist`/`item_blacklist_keys`; 新增
            `import_kangfu_into_item_blacklist(kf_items, item_pool=None)`(按同名秘籍导入, 实测功法名库
            1045 门 100% 都能在道具名库里找到同名 —— 秘籍本身就是道具) 与
            `kangfu_purge_blacklisted(text, targets, blacklist_keys)`(按容器开括号**降序**逐角色删,
            带「开括号处必须是 { 」的偏移健全检查, 偏移失效宁可不删也不乱删)。
            ② **圆桌对话框基类** `BlacklistDialog`(标题/名词/单位/说明/数据读写/候选来源/额外按钮由子类提供,
            表格+加入+删除+清空+备注即时写盘共用) → `KangFuBlacklistDialog` 与 `ItemBlacklistDialog`。
            ③ **一键清除**: 功法黑名单框「⚠ 清除已学…」→ 选角色范围(QInputDialog: 全部角色/主角/某个队友)
            → `MainWindow._purge_kangfu_blacklist(scope=None, confirm=True)`(worker 里删 + 汇总确认 +
            主线程刷新功法列表); 道具黑名单框「⚠ 清除包裹里…」→ `_purge_item_blacklist()`(从 self._items
            按黑名单名删除 + 局部删行/整表重建 + 置脏)。两者都只改内存, 点「保存」才写回。
            ④ **道具黑名单拦截**(与功法黑名单同款两道): 「＋ 新增道具…」「商店 → 添加卖品…」传
            `hidden_names=self._item_blocked_names()`(候选完全不显示), 确定后再按 `_item_blocked()`
            拦手动输入/自定义名并提示; 「一键添加全部道具」在 to_add 前过滤掉黑名单条目。
            ⑤ 主窗新增 `self._item_blacklist`/`_item_blocked()`/`_item_blocked_names()`/`_open_item_blacklist()`
            /`_char_friends`(读档时记录 [(group,key,名,scope)]) 与 `_char_scopes()`, 工具栏加「道具黑名单…」。
            核心 **287/287**(新增 16 条: 道具黑名单读写/归一化键/两名单文件互不干扰/导入同名秘籍(100% 命中、
            库外进 miss、空输入安全)/一键清除(多角色多段全删、非命中保留、括号配平、空黑名单与空目标原样返回、
            无中招不产生条目、偏移失效防御))、GUI **335/335**(新增 22 条: 道具黑名单框构造/导入同名秘籍/
            批量加入去重/备注/删除/清空/主窗加载/添加时拦下不重复添加/弹提示与 hidden_names/非黑名单照常加/
            一键添加全部不含黑名单/清除包裹/只改内存/无中招返回 0/一键清除已学(置脏、括号、列表刷新、再清为 0))、
            `_scaffold/e2e_kangfu.py` **56/56**(新增 6 条: 真实 8.6MB 档主角 998 门里删头尾两门不同段 →
            996, 未命中条目集合差 = 删除数、括号配平、再清无可删)、`_scaffold/e2e_chong_equip.py` 16/16。
...
def _direct_entries(text, open_i):
    """`_entry_values` 的**快路径**: 只枚举 open_i 处对象/数组的**直接**条目。

    返回 [(键 or None, 值起点, 值终点)] —— 值为对象/数组时含外层括号; 终点是「条目段末」,
    非末条的终点含尾随逗号(与 `_entry_values` 的区间语义一致)。**不做逐条切片、不跑正则**,
    只用配对表 + `str.find` + `_string_end`(全 C 级)。

    ★ 为什么需要: 单个队友对象 ~150KB/56 字段, `_entry_values` 在它上面要 ~33ms(逐条 seg 切片 +
      `_key_value_offset` 正则), 而 `friend_blocks` 每个队友要跑两次(查 _AbName / 读 _Name)
      → 20 名队友实测 1.6s(打开「队伍管理…」会明显卡一下); 本函数把这段降到十几 ms。
    配对表缺失/异常时返回 None, 调用方自行回退 `_entry_values`。
    """
    poss, chars, pairs = _get_struct_index(text)
    np = len(poss)
    i0 = bisect_left(poss, open_i)
    if pairs is None or i0 >= np or poss[i0] != open_i:
        return None
    close_pi = pairs[i0]
    if close_pi < 0:
        return None
    ...
    for b in range(len(bounds) - 1):
        ...
        if not is_obj:
            ...
            while vs < a1 and text[vs] in " \t\r\n":
                vs += 1
            out.append((None, vs, a1))
            continue
        q = text.find('"', a0)
        if q < 0 or q >= a1:
            continue
        k_end = _string_end(text, q)
        if k_end > a1:
            continue
        c = k_end
        while c < a1 and text[c] in " \t":
            c += 1
        if c >= a1 or text[c] != ":":
            continue
        c += 1
        while c < a1 and text[c] in " \t\r\n":
            c += 1
        out.append((_es3_unescape_name(text[q + 1:k_end - 1]), c, a1))
    return out


def _read_str_value(text, val_start):
    """读 val_start 处(可含前导空白)的字符串值; 不是字符串返回 None。"""
    i = val_start
    n = len(text)
    ...
    if i >= n or text[i] != '"':
        return None
    e = _string_end(text, i)
    return _es3_unescape_name(text[i + 1:e - 1])
...
# ==================== 黑名单(功法黑名单.json / 道具黑名单.json, v2.15.0 / v2.16.0) ====================
# 用途: 记录「一学就卡死 / 一拿到就出问题」的功法与道具(用户实测)。加进去以后:
#   ① 「＋ 添加功法…」/「＋ 新增道具…」的候选里**完全不显示**(眼不见心不烦, 不会手滑选中);
#   ② 主角/队友不会被加功法, 角色包裹也不会被加道具(即便名字被别处传进来, 也会被拦下);
#   ③ 已学功法列表会标注「★ 在黑名单」; 并可**一键清除**角色身上/包裹里已在黑名单的条目。
# 用户意图: 先把已知会卡死的列进去避开, 以后有空再逐条验证到底是「剧情还没到」还是别的原因
#   —— 所以每条可以写备注(现象/猜测), 记在 json 里。
# 数据与存档无关(程序级), 存程序目录, 换档/重开程序都生效。
# 实测(v2.16.0): 功法名库 1045 门 **全部**都能在道具名库里找到同名秘籍(秘籍本身就是道具),
#   所以「把黑名单功法的秘籍一并列入道具黑名单」是可靠的同名导入。

def _load_blacklist_file(path):
    """通用黑名单读取: 返回 [{"name": 名, "note": 备注}](保持文件顺序); 缺失/损坏返回 []。
...
def _save_blacklist_file(items, path, note):
    """通用黑名单原子写入(临时文件 + os.replace); 返回 (是否成功, 路径或错误消息)。"""
...
def _blacklist_keys(items):
    """黑名单的归一化名集合(判「这个名字在黑名单里吗」用)。"""
...
def load_kangfu_blacklist(path=None):
    """读功法黑名单: [{"name": 名, "note": 备注}]; 缺失/损坏返回 []。"""
    return _load_blacklist_file(path or get_kangfu_blacklist_path())


def save_kangfu_blacklist(items, path=None):
    """把功法黑名单原子写入 json; 返回 (是否成功, 路径或错误消息)。"""
    return _save_blacklist_file(
        items, path or get_kangfu_blacklist_path(),
        "本文件由程序「功法黑名单…」维护: 列入的功法在添加候选里不显示、不会被加给任何角色。")


def kangfu_blacklist_keys(items):
    """功法黑名单的归一化名集合(判「这门功法在黑名单里吗」用)。"""
    return _blacklist_keys(items)


# ---- 道具黑名单(v2.16.0) ----
# 主要用于「黑名单功法的同名秘籍」—— 秘籍也是道具(ItemHad), 拿到/添加同名秘籍同样可能触发问题;
# 也可以把任何会导致卡死的道具列进来: 新增道具/一键添加全部道具/商店添加卖品 都不会再出现它,
# 并能一键清除主角包裹里已在黑名单的道具。
def get_item_blacklist_path():
    """道具黑名单路径(程序目录/道具黑名单.json)。"""
    return os.path.join(app_dir(), "道具黑名单.json")


def load_item_blacklist(path=None):
    """读道具黑名单: [{"name": 名, "note": 备注}]; 缺失/损坏返回 []。"""
    return _load_blacklist_file(path or get_item_blacklist_path())


def save_item_blacklist(items, path=None):
    """把道具黑名单原子写入 json; 返回 (是否成功, 路径或错误消息)。"""
    return _save_blacklist_file(
        items, path or get_item_blacklist_path(),
        "本文件由程序「道具黑名单…」维护: 列入的道具在添加/上架候选里不显示、也不会被加进包裹。")


def item_blacklist_keys(items):
    """道具黑名单的归一化名集合(判「这个道具在黑名单里吗」用)。"""
    return _blacklist_keys(items)


def import_kangfu_into_item_blacklist(kf_items, item_pool=None):
    """把功法黑名单里的功法, 按**同名秘籍**并入道具黑名单。

    返回 (可加入的道具名列表, 道具库里找不到的名字列表)。item_pool=None 时用 道具名库;
    实测功法名库 1045 门 100% 都能在道具名库里找到同名秘籍, 所以正常「找不到」应为空。
    名字比较用 `_kangfu_norm`(去空白/零宽字符)。
    """
    pool = set(item_pool) if item_pool is not None else set(load_item_names())
    pool_norm = {}
    for n in pool:
        pool_norm.setdefault(_kangfu_norm(n), n)
    out = []
    miss = []
    for it in (kf_items or []):
        nm = (it.get("name") or "").strip()
        if not nm:
            continue
        real = pool_norm.get(_kangfu_norm(nm))
        if real is None:
            miss.append(nm)
        else:
            out.append(real)
...
def kangfu_purge_blacklisted(text, targets, blacklist_keys):
    """把若干角色身上「已在黑名单里」的功法删掉(一键清除用)。

    targets: [(角色标签, 容器开括号或 None)]; blacklist_keys: 归一化名集合(见 `_blacklist_keys`).
    返回 (新文本, [(角色标签, ["[段标题]名", ...]), ...])。
    ★ 按容器开括号**降序**处理: 删条目会改变后面的偏移, 先处理靠后的角色, 前面角色的偏移才不失效。
    """
    if not text or not targets or not blacklist_keys:
        return text, []
    nt = text
    removed = []
    todo = sorted([t for t in targets if t and t[1] is not None], key=lambda x: -x[1])
    for label, co in todo:
        # 健全检查: 容器开括号处必须是 `{` —— 偏移失效(文本被别的改动改过)时跳过该角色,
        # 宁可不删也不在错位置下刀
        if co >= len(nt) or nt[co:co + 1] != "{":
            continue
        hits = [(seg, nm) for seg, _t, nm, _ks, _vs, _vc in kangfu_records(nt, co)
                if _kangfu_norm(nm) in blacklist_keys]
        if not hits:
            continue
        nt, gone = kangfu_del(nt, co, hits)
        if gone:
            removed.append((label, gone))
    return nt, removed
...
def friend_blocks(text):
    ...
    ★ 用 `_direct_entries` 快路径枚举(每个队友只扫一遍直接字段): 真实 10MB 档 20 名队友
      从 ~1.6s 降到 ~0.1s(`_entry_values` 要在 150KB 的队友对象上逐条切片并跑正则)。
    """
    out = []
    val = friend_value_open(text)
    if val is None:
        return []
    top = _direct_entries(text, val)
    if top is None:
        top = [(kd, vs, _ve) for kd, vs, _ve, _vt in _entry_values(text, val)]
    for group, gs, _ge in top:
        if text[gs:gs + 1] != "{":
            continue
        ents = _direct_entries(text, gs)
        if ents is None:
            ents = [(kd, vs, _ve) for kd, vs, _ve, _vt in _entry_values(text, gs)]
        for key, vs, _ve in ents:
            if key is None or text[vs:vs + 1] != "{":
                continue
            ab, nm = friend_obj_names(text, vs)
            if ab is None:
                continue
...
# ==================== 队友「加入队伍 / 改为离队」(v2.17.0) ====================
# 实测(20 个真实存档: custom0/1/4/9/13..17、quick1~3、Auto1~3、note1~3…):
# saveFriendData.value 下用两个**字典容器**互斥地装队友属性对象 ——
#   「在队 AddFriends」与「离队 LeaveFriends」; 另有三个数组 LeaveAllFriends /
#   LeaveAllFriendsBattle / AddAllFriends(实测恒为空) 与一个数组 BattleFriend。
# · 同一名队友在两个容器里的字段**完全一致**(实测 56 个字段, 无一多无一少; 也无一字段只在一边),
#   所以「入队 / 离队」只需把这个队友对象**整条**从旧容器搬进新容器 —— 等级/经验/技能/装备/
#   功法/外观等全部字段逐字节保留, 游戏里看起来就是同一个人换了队伍状态。
# · BattleFriend 是**出战队伍**(实测最多 3 人, 且恒为 AddFriends 的子集):
#   入队时**不**往里加(出战阵容请在游戏里点选), 但**离队时会把它摘掉**,
#   免得留下「出战名单里的人是离队状态」的悬空引用。
FRIEND_GROUP_IN = "AddFriends"        # 在队
FRIEND_GROUP_OUT = "LeaveFriends"     # 离队
FRIEND_GROUP_ORDER = (FRIEND_GROUP_IN, FRIEND_GROUP_OUT)
FRIEND_BATTLE_FIELD = "BattleFriend"  # 出战队伍(最多 3 人): 队友键的字符串数组
# 入队/离队 的目标写法别名(界面按钮传中文, 纯函数调用传容器名)
FRIEND_GROUP_ALIAS = {"在队": FRIEND_GROUP_IN, "加入队伍": FRIEND_GROUP_IN,
                      "离队": FRIEND_GROUP_OUT, "改为离队": FRIEND_GROUP_OUT}


def friend_group_label(group):
    """队友容器名 → 中文说明(未知容器原样返回)。"""
    return FRIEND_GROUP_LABELS.get(group, group or "")


def friend_value_open(text):
    """快速定位 saveFriendData.value 的值开括号; 找不到返回 None。

    ★ `_locate_container(["saveFriendData","value"])` 的第一层是「按行正则扫整档」
      (10MB/28 万行实测 ~70ms), 每层都要走一遍; 而 `"saveFriendData"` 这个段名全档唯一,
      直接 `str.find` + 跳到值括号即可(微秒级)。找不到/不唯一时回退原路径。
    """
    key = '"saveFriendData"'
    i = text.find(key)
    if i >= 0 and text.find(key, i + 1) < 0:
        j = text.find('"value"', i + len(key), i + len(key) + 200)
        if j > 0:
            k = text.find(":", j)
            if 0 < k < j + 40:
                k += 1
                ...
                if text[k:k + 1] == "{":
                    return k
    return _locate_container(text, ["saveFriendData", "value"])


def friend_obj_names(text, obj_open):
    """读一个队友对象里的**直接**字段 _AbName / _Name(单遍快路径); 返回 (ab, name)。

    ab 为 None 表示这不是队友对象(没有直接 _AbName)。只扫直接字段, 不会误读内部嵌套对象里的
    同名字段(如 CharacterHad 里也有 `_Name`)。
    """
    ents = _direct_entries(text, obj_open)
    if ents is None:
        ents = [(kd, vs, _ve) for kd, vs, _ve, _vt in _entry_values(text, obj_open)]
    ab = nm = None
    for kd, vs, _ve in ents:
        if kd == "_AbName":
            ab = _read_str_value(text, vs)
        elif kd == "_Name":
            nm = _read_str_value(text, vs)
    return ab, nm


def friend_container_opens(text):
    """一次定位 saveFriendData.value 下**全部**对象型容器: {容器名: 值开括号}。

    比逐个调 friend_container_open 省掉重复的 value 定位。
    """
    val = friend_value_open(text)
    if val is None:
        return {}
    ents = _direct_entries(text, val)
    if ents is None:
        ents = [(kd, vs, _ve) for kd, vs, _ve, _vt in _entry_values(text, val)]
    return {kd: vs for kd, vs, _ve in ents
            if kd is not None and text[vs:vs + 1] == "{"}


def _dict_key_start(text, val_open):
    """由「值」的开括号反推该条目**键**的起点(含左引号); 推不出来返回 None。

    形如 `"luxi":{` / `"luxi" : {` —— 从 `{` 往前跳空白、`:`, 再跳空白, 撞到左引号后
    再向前找到配对的左引号(键内是纯 ASCII, 不跨行, 所以一路上遇到换行就放弃)。
    """
    i = val_open - 1
...
def friend_container_open(text, group):
    """定位 saveFriendData.value 内某个队友容器(如 AddFriends/LeaveFriends)的值开括号。

    不存在/不是对象时返回 None。只扫 saveFriendData.value 这一层(基于结构索引, 不逐字符)。
    """
    return friend_container_opens(text).get(group)


def friend_entries(text, group):
    """枚举某队友容器里的条目: [(键, 键起点, 值开括号, 值闭括号)], 顺序同文件。"""
    open_i = friend_container_open(text, group)
    if open_i is None:
        return []
    close = _matching_end(text, open_i)
    out = []
    ents = _direct_entries(text, open_i)
    if ents is None:
        ents = [(kd, vs, _ve) for kd, vs, _ve, _vt in _entry_values(text, open_i)]
    for kd, vs, _ve in ents:
        if kd is None or text[vs:vs + 1] != "{":
            continue
        ks = _dict_key_start(text, vs)
        if ks is None:
            continue
        vc = _matching_end(text, vs)
        if vc is None or vc > close:
            continue
        out.append((kd, ks, vs, vc))
    return out


def friends_move(text, moves, sync_battle=True):
    """把队友在「在队(AddFriends)」与「离队(LeaveFriends)」之间整条搬移。

    moves = [(队友键, 目标容器)] —— 目标容器可传容器名(AddFriends/LeaveFriends)或
    中文别名(在队/离队); 同一键重复出现只处理第一次。

    返回 (新文本, 实际搬移的 [(键, 目标容器)], 跳过的 [(键, 原因)])。
    · **只换容器, 队友对象内容逐字节不动**(等级/技能/装备/功法/外观全部原样);
    · 两个容器里**其它队友与整档其它段落**逐字节不变(只重建这两个容器的体内文本);
    · sync_battle=True 时: 从「在队」搬去「离队」的队友会**同时从出战队伍 BattleFriend 摘掉**
      (实测 BattleFriend 恒为 AddFriends 的子集); 入队时不往 BattleFriend 里加。
    """
    if not moves:
        return text, [], []
    ent = {g: friend_entries(text, g) for g in FRIEND_GROUP_ORDER}
    where = {}
    for g in FRIEND_GROUP_ORDER:
        for kd, ks, vs, vc in ent[g]:
            where.setdefault(kd, (g, ks, vs, vc))
    removed = {g: set() for g in FRIEND_GROUP_ORDER}
    insert = {g: [] for g in FRIEND_GROUP_ORDER}
    done, skipped, out_of_team = [], [], []
    seen = set()
    for key, to in moves:
        key = (key or "").strip()
        to = FRIEND_GROUP_ALIAS.get(to, to)
        if not key or to not in FRIEND_GROUP_ORDER or key in seen:
            continue
        cur = where.get(key)
        if cur is None:
            skipped.append((key, "存档里没有这名队友"))
            continue
        if cur[0] == to:
            skipped.append((key, "已经在「%s」里" % friend_group_label(to)))
            continue
        if any(kd == key for kd, _a, _b, _c in ent[to]):
            skipped.append((key, "「%s」里已有同名队友" % friend_group_label(to)))
            continue
        seen.add(key)
        removed[cur[0]].add(key)
        insert[to].append(text[cur[1]:cur[3] + 1])   # 键 + `:` + 对象, 原样搬过去
        done.append((key, to))
        if cur[0] == FRIEND_GROUP_IN and to == FRIEND_GROUP_OUT:
            out_of_team.append(key)
    if not done:
        return text, [], skipped
    # ★ 只做「最小跨度」改写: 重建整段容器体会产生 MB 级替换文本, `_apply_subs` 打索引补丁时
    #   要逐引号重扫替换文本(实测 2.4MB → 1.6s); 这里把改动拆成
    #   ① 最后一个保留条目之后的尾部区域一次替换(内含被删的尾部条目 + 新增条目);
    #   ② 尾区之前的「连续被删段」各自一个最小跨度(连同它前后那个逗号)。
    opens = friend_container_opens(text)
    subs = []
    for g in FRIEND_GROUP_ORDER:
        if not removed[g] and not insert[g]:
            continue
        ents = ent[g]
        open_i = opens.get(g)
        if open_i is None:
            continue
        close = _matching_end(text, open_i)
        # 条目之间的原文分隔(真实档就是 ","), 新增条目沿用同一种分隔, 与游戏写出来的一致
        sep = text[ents[0][3] + 1:ents[1][1]] if len(ents) >= 2 else ","
        keep_idx = [i for i, e in enumerate(ents) if e[0] not in removed[g]]
        last_keep = keep_idx[-1] if keep_idx else None
        tail_start = ents[last_keep][3] + 1 if last_keep is not None else open_i + 1
        need_tail = bool(insert[g]) or (last_keep is None and bool(removed[g])) \
            or (last_keep is not None and last_keep < len(ents) - 1)
        if need_tail:
            orig_tail = text[ents[-1][3] + 1:close] if ents else text[open_i + 1:close]
            # 往「已有条目」后面追加要补分隔逗号; 容器原本为空(或全删光)则首条直接内联在 `{` 之后
            pre = sep if (insert[g] and last_keep is not None) else ""
            new_tail = pre + sep.join(insert[g]) + orig_tail
            subs.append((tail_start, close, new_tail))
        if last_keep is None:
            continue          # 全部条目都在尾区里处理掉了
        i = 0
        while i < len(ents):
            if ents[i][0] not in removed[g]:
                i += 1
                continue
            j = i
            while j + 1 < len(ents) and ents[j + 1][0] in removed[g]:
                j += 1
            if j < last_keep:      # 该连续段整体在最后一个保留条目之前
                if j < len(ents) - 1:
                    nxt = ents[j][3] + 1
                    end = nxt + 1 if text[nxt:nxt + 1] == "," else nxt
                    subs.append((ents[i][1], end, ""))
                else:
                    s = ents[i - 1][3] + 1 if i > 0 else ents[i][1]
                    while s < len(text) and text[s] in " \t":
                        s += 1
                    subs.append((s, ents[j][3] + 1, ""))
            i = j + 1
    nt = _apply_subs(text, subs)
    if sync_battle and out_of_team:
        for key in out_of_team:
            nt = remove_str_array(nt, ["saveFriendData", "value"], FRIEND_BATTLE_FIELD, key)
    return nt, done, skipped


def friends_battle_list(text):
    """读出战队伍(BattleFriend)的队友键列表; 无该字段/为空返回 []。"""
    return get_str_array(text, ["saveFriendData", "value"], FRIEND_BATTLE_FIELD)
...
class BlacklistDialog(QDialog):
    """黑名单管理对话框基类(功法黑名单 / 道具黑名单共用)。

    用途: 记录「一学就卡死 / 一拿到就出问题」的功法或道具。加进黑名单以后:
      ① 对应的「添加…」候选里**完全不显示**(不会手滑选中);
      ② 主角/队友不会被加功法, 包裹也不会被加道具(即便名字从别处传进来, 主窗也会再拦一道);
      ③ 已学功法列表里标「★ 在黑名单」; 并可**一键清除**角色身上/包裹里已在黑名单的条目。
    支持批量加入(分类库多选/全选)、批量移出(多选)、逐条备注(记现象/猜测原因 —— 便于以后
    逐条验证是「剧情还没到」还是别的原因)。改动**即时写盘**, 不用点确定。

    子类提供: 窗口标题/说明文案/名词/数据读写函数/候选来源/额外按钮。
    """
    ...
    ADD_TIP = ""
    COUNT_EMPTY = "黑名单为空"
    PURGE_LABEL = None      # 「一键清除」按钮文案(None=不显示)
    PURGE_TIP = ""
    ...
    def _make_add_dialog(self):
        """构造「从库中添加」对话框(子类实现, 返回带 exec()/chosen_names() 的对话框)。"""
        raise NotImplementedError

    def _make_extra_buttons(self, row):
        """子类可追加按钮到按钮行(默认无)。"""
        return

    def __init__(self, parent=None, items=None, path=None, on_purge=None):
        super().__init__(parent)
        ...
        self.table.setHorizontalHeaderLabels(["%s名" % self.NOUN, "备注(现象 / 猜测原因)"])
        ...
        row.addStretch(1)
        b_add = QPushButton(self.ADD_LABEL)
        b_add.setToolTip(self.ADD_TIP)
        b_add.clicked.connect(self._on_add)
        b_del = QPushButton("－ 删除选中")
        b_del.setToolTip("把选中的条目移出黑名单(移出后它们又会出现在添加候选里)")
        b_del.clicked.connect(self._on_del)
        b_clear = QPushButton("清空全部")
        b_clear.setToolTip("清空整个黑名单(所有条目都会重新出现在添加候选里)")
        b_clear.clicked.connect(self._on_clear)
        for b in (b_add, b_del, b_clear):
            row.addWidget(b)
        self._make_extra_buttons(row)
        if self.PURGE_LABEL and on_purge is not None:
            b_purge = QPushButton(self.PURGE_LABEL)
            b_purge.setToolTip(self.PURGE_TIP)
            b_purge.clicked.connect(lambda: self._on_purge())
            row.addWidget(b_purge)
        root.addLayout(row)
    ...
    def _reload(self):
        ...
        try:
            ...
            for r, it in enumerate(self.items):
                ...
                c0.setToolTip("不会出现在添加候选里, 也不会被加给任何角色")
                t.setItem(r, 0, c0)
    ...
    def _update_count(self):
        n = len(self.items)
        if n:
            self.count_label.setText("黑名单: %d %s" % (n, self.UNIT))
        else:
            self.count_label.setText("黑名单为空 —— 点右边「%s」把已知会出问题的%s加进来"
                                     % (self.ADD_LABEL, self.NOUN))
    ...
    def _save(self):
        ...
        ok, info = self._save_items(self.items)
        if not ok:
            QMessageBox.warning(self, "提示", "写入黑名单文件失败:\n%s" % info)

    def add_names(self, names):
        """把一批名字加入黑名单(去重/跳过空名); 返回实际加入的个数。"""
    ...
    def _on_add(self):
        """从库中批量加入黑名单(复用「新增道具」的分类树/搜索/多选/全选)。"""
        dlg = self._make_add_dialog()
        if not dlg.exec():
            return
        self.add_names(dlg.chosen_names())

    def _on_del(self):
        rows = sorted({i.row() for i in self.table.selectedItems()}, reverse=True)
        if not rows:
            QMessageBox.information(self, "提示",
                                    "请先在列表里选中要移出黑名单的%s(可按住 Ctrl/Shift 多选)"
                                    % self.NOUN)
            return
        ...
        if QMessageBox.question(
                self, "确认移出",
                "把 %d 个%s移出黑名单?\n%s\n\n(移出后它们会重新出现在添加候选里)"
                % (len(names), self.NOUN, "、".join(names[:10]))
                + (" 等 %d 个" % len(names) if len(names) > 10 else "")
    ...
    def _on_clear(self):
        ...
        if QMessageBox.question(self, "确认清空",
                                "清空整个黑名单(共 %d 个%s)?\n"
                                "清空后这些%s会重新出现在添加候选里。"
                                % (len(self.items), self.NOUN, self.NOUN)
                                ) != QMessageBox.StandardButton.Yes:
            return
        self.items = []
        self._save()
...
class KangFuBlacklistDialog(BlacklistDialog):
    """功法黑名单管理(程序目录/功法黑名单.json, v2.15.0)。

    列入的功法: 「＋ 添加功法…」候选里不显示、不会被加给主角/队友; 已学列表悬停标「★ 在黑名单」。
    额外提供「⚠ 清除已学…」(v2.16.0): 把主角/队友**已经学了**的黑名单功法从存档里删掉
    —— 已经中招的存档可以一键清干净(删除只写在内存, 点工具栏「保存」才写回文件)。
    """

    TITLE = "功法黑名单(已知会卡死的功法)"
    NOUN = "功法"
    UNIT = "门"
    TIP = ("黑名单里的功法:「＋ 添加功法…」候选里不会显示, 也不会被加给主角/队友。\n"
           "用途: 把已知会让游戏卡死的功法先记下来避开, 以后有空再逐条确认到底是「剧情还没到」\n"
           "还是别的原因 —— 可在「备注」列写下现象/猜测。改动即时保存到 程序目录/功法黑名单.json,\n"
           "与当前存档无关(换档、重开程序都生效)。\n"
           "★「⚠ 清除已学…」可把主角/队友**已经学了**的黑名单功法从当前存档里删掉(先选角色范围)。")
    ADD_LABEL = "＋ 从功法库添加…"
    ADD_TIP = ("打开功法库(内功/武功/绝技/轻功 分类树 + 中文/拼音搜索)多选/全选加入黑名单;\n"
               "已在本黑名单里的会灰显跳过")
    PURGE_LABEL = "⚠ 清除已学…"
    PURGE_TIP = ("把主角/队友身上「已经学了」的黑名单功法删掉(可选范围: 全部角色 / 主角 / 某个队友);\n"
                 "删除只改内存, 点工具栏「保存/另存为/复制结果」才写回存档")

    def _default_path(self):
        return get_kangfu_blacklist_path()

    def _load_items(self):
        return load_kangfu_blacklist(self._path)

    def _save_items(self, items):
        return save_kangfu_blacklist(items, self._path)

    def _make_add_dialog(self):
        return AddItemDialog(self, existing={it.get("name") for it in self.items},
                             cat_data=load_kangfu_categories(),
                             fallback_names=load_kangfu_names(),
                             title="加入功法黑名单", noun="功法", show_count=False)


class ItemBlacklistDialog(BlacklistDialog):
    """道具黑名单管理(程序目录/道具黑名单.json, v2.16.0)。

    列入的道具: 「＋ 新增道具…」「一键添加全部道具」「商店 → 添加卖品…」的候选里不显示、
    也不会被加进包裹; 额外提供:
      · 「↧ 从功法黑名单导入同名秘籍」—— 把黑名单功法的**同名秘籍**(秘籍本身就是道具)一并列为
        黑名单道具(实测 1045 门功法 100% 都能在道具名库里找到同名秘籍);
      · 「⚠ 清除包裹里…」—— 把主角包裹(ItemHad)里已在黑名单的道具一键删除。
    """

    TITLE = "道具黑名单(已知会出问题的道具/秘籍)"
    NOUN = "道具"
    UNIT = "个"
    TIP = ("黑名单里的道具:「＋ 新增道具…」「一键添加全部道具」「商店 → 添加卖品…」里都不会出现,\n"
           "也不会被加进主角包裹(手动输入的名字也会被拦下)。\n"
           "主要用途: 把黑名单功法的**同名秘籍**一并列进来(秘籍本身也是道具, 拿到同样可能触发问题)\n"
           "—— 点「↧ 从功法黑名单导入同名秘籍」即可; 也可以把任何会导致卡死的道具加进来。\n"
           "改动即时保存到 程序目录/道具黑名单.json, 与当前存档无关。\n"
           "★「⚠ 清除包裹里…」可把主角包裹(ItemHad)里已在黑名单的道具一键删掉。")
    ADD_LABEL = "＋ 从道具库添加…"
    ADD_TIP = ("打开道具库(按分类树浏览 + 中文/拼音搜索)多选/全选加入黑名单;\n"
               "已在本黑名单里的会灰显跳过")
    PURGE_LABEL = "⚠ 清除包裹里…"
    PURGE_TIP = ("把主角包裹(ItemHad)里已在黑名单的道具删掉;\n"
                 "删除只改内存, 点工具栏「保存/另存为/复制结果」才写回存档")

    def _default_path(self):
        return get_item_blacklist_path()

    def _load_items(self):
        return load_item_blacklist(self._path)

    def _save_items(self, items):
        return save_item_blacklist(items, self._path)

    def _make_add_dialog(self):
        return AddItemDialog(self, existing={it.get("name") for it in self.items},
                             title="加入道具黑名单", noun="道具", show_count=False)

    def _make_extra_buttons(self, row):
        b_imp = QPushButton("↧ 从功法黑名单导入同名秘籍")
        b_imp.setToolTip(
            "把「功法黑名单…」里的每一门功法, 按**同名秘籍**加进道具黑名单\n"
            "(秘籍本身就是道具, 拿到手同样可能触发卡死问题)\n"
            "实测 1045 门功法 100% 都能在道具名库里找到同名秘籍")
        b_imp.clicked.connect(self._on_import_kangfu)
        row.addWidget(b_imp)

    def _on_import_kangfu(self):
        """把功法黑名单的功法按同名秘籍导入道具黑名单。"""
        kf = load_kangfu_blacklist()
        if not kf:
            QMessageBox.information(
                self, "提示",
                "功法黑名单是空的 —— 先到工具栏「功法黑名单…」把已知会卡死的功法加进去,\n"
                "再回来点这个按钮就能把它们的同名秘籍一并列进来。")
            return
        names, miss = import_kangfu_into_item_blacklist(kf)
        added = self.add_names(names)
        msg = "已从功法黑名单(%d 门)导入同名秘籍: 新增 %d 个" % (len(kf), added)
        if added < len(names):
            msg += "(其余 %d 个已在道具黑名单里)" % (len(names) - added)
        if miss:
            msg += "\n以下 %d 门在道具名库里找不到同名秘籍: %s" % (
                len(miss), "、".join(miss[:8]))
        QMessageBox.information(self, "导入完成", msg)


class PartyDialog(QDialog):
    """队伍管理(v2.17.0): 一次看全部队友的 在队/离队 状态, 多选批量「加入队伍 / 改为离队」。

    只负责「选人 + 点按钮」, 真正的搬移交给主窗回调 `on_apply(moves)`
    (后台线程搬移 + 重建角色页), 回调返回 `(True, 新记录)` 或 `(False, 跳过原因文本)`;
    成功后用新记录刷新本表, 所以可以连着调整多次。
    """

    def __init__(self, parent=None, records=None, on_apply=None, battle=None):
        super().__init__(parent)
        self._records = list(records or ())       # [(容器, 队友键, 显示名)]
        self._on_apply = on_apply
        self._battle = set(battle or ())          # 出战队伍(BattleFriend)里的队友键
        self.setWindowTitle("队伍管理(加入队伍 / 改为离队)")
        self.setMinimumSize(680, 470)
        root = QVBoxLayout(self)
        tip = QLabel(
            "把队友在「在队 AddFriends」与「离队 LeaveFriends」之间切换(可 Ctrl/Shift 多选后批量)。\n"
            "★ 是整条搬移: 等级/经验/技能/装备/功法/外观等字段原样保留(只换容器, 内容逐字节不动)。\n"
            "★ 从「在队」改为「离队」时, 若该队友正在出战队伍(BattleFriend, 实测最多 3 人)里会一并摘掉;\n"
            "   加入队伍时不会自动进出战阵容 —— 出战 3 人请在游戏里点选(这里只改「在队伍 / 离队」归属)。\n"
        ...
        self.table.setHorizontalHeaderLabels(["队友", "英文键", "队伍状态", "出战队伍"])
        _hdr = self.table.horizontalHeader()
        ...
        self.table.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows)
        self.table.setSelectionMode(QAbstractItemView.SelectionMode.ExtendedSelection)
        self.table.setToolTip("选中一行或多行(按住 Ctrl/Shift)后点下方按钮批量切换队伍状态")
        root.addWidget(self.table, 1)
        ...
        b_all.setToolTip("选中表格里全部队友")
        b_all.clicked.connect(lambda: self.table.selectAll())
        b_none = QPushButton("清空选择")
        b_none.clicked.connect(self.table.clearSelection)
        b_in = QPushButton("✓ 加入队伍(在队)")
        b_in.setToolTip("把选中的队友整条搬到「在队 AddFriends」")
        b_in.clicked.connect(lambda: self._apply(FRIEND_GROUP_IN))
        b_out = QPushButton("✗ 改为离队")
        b_out.setToolTip("把选中的队友整条搬到「离队 LeaveFriends」(在出战队伍里的会一并摘掉)")
        b_out.clicked.connect(lambda: self._apply(FRIEND_GROUP_OUT))
        for b in (b_all, b_none, b_in, b_out):
            row.addWidget(b)
        root.addLayout(row)
    ...
    def _reload(self):
        """按 self._records 重建表格(不改选择)。"""
        t = self.table
        t.setRowCount(len(self._records))
        for r, (group, key, nm) in enumerate(self._records):
            c0 = QTableWidgetItem(nm or key)
            t.setItem(r, 0, c0)
            t.setItem(r, 1, QTableWidgetItem(key))
            st = friend_group_label(group)
            c2 = QTableWidgetItem(st if group == FRIEND_GROUP_IN else "%s(不在队伍)" % st)
            c2.setToolTip("在队 = 游戏里会跟随你; 离队 = 还认识但不在队伍里")
            t.setItem(r, 2, c2)
            c3 = QTableWidgetItem("★ 出战" if key in self._battle else "")
            c3.setToolTip("出战队伍 BattleFriend(最多 3 人)—— 在游戏里点选出战 3 人")
            t.setItem(r, 3, c3)
        n_in = sum(1 for g, _k, _n in self._records if g == FRIEND_GROUP_IN)
        self.count_label.setText("队友共 %d 人 · 在队 %d · 离队 %d"
                                 % (len(self._records), n_in, len(self._records) - n_in))

    def _selected_keys(self):
        rows = sorted({i.row() for i in self.table.selectedItems()})
        return [self._records[r][1] for r in rows if 0 <= r < len(self._records)]

    def _apply(self, to_group):
        """把选中的队友批量搬去 to_group(交主窗执行)。"""
        keys = self._selected_keys()
        if not keys:
            QMessageBox.information(self, "提示", "请先在表格里选中队友(可按住 Ctrl/Shift 多选)")
            return
        if self._on_apply is None:
            return
        ok, payload = self._on_apply([(k, to_group) for k in keys])
        if not ok:
            QMessageBox.information(self, "提示", payload if isinstance(payload, str)
                                    else "调整队伍失败")
            return
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # 功法黑名单(v2.15.0) / 道具黑名单(v2.16.0): 程序级数据(程序目录 json), 列入的条目在
        # 添加候选里不显示、也不会被加给主角/队友/包裹 —— 用于先避开已知会卡死的功法与秘籍道具
        self._kangfu_blacklist = load_kangfu_blacklist()
        self._item_blacklist = load_item_blacklist()
        # 当前档的角色列表 [(group, key, 显示名, scope)]: 黑名单「清除已学」时选角色范围用(读档时填)
        self._char_friends = []
    ...
    def _build_ui(self):
        ...
        # 队伍管理(v2.17.0): 把离队的队友加入队伍 / 把在队的改成离队(整条搬移, 内容不变)
        self.act_party = _add_act("队伍管理…", self._open_party_dialog)
        self.act_party.setToolTip(
            "一次看全部队友的 在队/离队 状态, 可多选批量「✓ 加入队伍(在队)」/「✗ 改为离队」;\n"
            "搬移是整条搬(只换容器): 等级/技能/装备/功法等字段原样保留;\n"
            "改为离队时若该队友在出战队伍(BattleFriend, 最多 3 人)里会一并摘掉")
        # 功法黑名单(v2.15.0) / 道具黑名单(v2.16.0): 把已知会卡死的功法与秘籍道具记下来避开
        # (程序级数据, 与存档无关; 两个框里都能「一键清除」角色身上/包裹里已中招的条目)
        ...
        self.act_kangfu_black.setToolTip(
            ...
            "可批量加入/移出、逐条写备注, 还能「⚠ 清除已学…」把角色身上已中招的功法删掉")
        self.act_item_black = _add_act("道具黑名单…", self._open_item_blacklist)
        self.act_item_black.setToolTip(
            "列出已知会出问题的道具(主要是黑名单功法的**同名秘籍**):\n"
            "新增道具/一键添加全部/商店添加卖品 都不会再出现它们, 也不会被加进包裹;\n"
            "可从功法黑名单一键导入同名秘籍, 并能「⚠ 清除包裹里…」删掉已中招的道具")
    ...
    def _on_shop_add(self):
        ...
        dlg = AddItemDialog(self, existing=existing, default_count=999,
                            ...
                            hidden_names=self._item_blocked_names())   # 道具黑名单: 候选里不显示
        dlg.setWindowTitle("为 %s 添加卖品" % ab)
        if not dlg.exec():
            return
        raw_names = list(dict.fromkeys(dlg.chosen_names()))
        iblk = self._item_blocked()
        names = [n for n in raw_names if _kangfu_norm(n) not in iblk]
        n_item_blk = len(raw_names) - len(names)
        ...
        if not names:
            if blocked:
                ...
                self.statusBar().showMessage(
                    "选中的 %d 个道具在「道具黑名单」里, 已跳过(已知会卡死)" % n_item_blk, 5000)
            return
        text = self._text
    ...
    def _apply_char_model(self, model, progress=False):
        ...
        self._friend_pages = {}    # 队友页登记(key → scope/页头/队伍状态控件), 供入队/离队就地刷新
        ...
        self._char_friends = [(f.get("group", "AddFriends"), f.get("key"),
                               f.get("name") or f.get("key"),
                               ["saveFriendData", "value", f.get("group", "AddFriends"),
                                f.get("key")])
                              for f in friends]
        ...
        for f in friends:
            ...
            self.tabs.addTab(self._build_friend_page(group, f.get("key"), disp,
                             ...
                             self._friend_tab_text(group, disp))
    ...
    def _open_kangfu_blacklist(self):
        ...
        框内「⚠ 清除已学…」回调到 `_purge_kangfu_blacklist_ui`(先选角色范围再删)。
        """
        dlg = KangFuBlacklistDialog(self, items=self._kangfu_blacklist,
                                    on_purge=self._purge_kangfu_blacklist_ui)
        dlg.exec()
        self._kangfu_blacklist = load_kangfu_blacklist()
    ...
    # ---- 道具黑名单(v2.16.0): 主要是黑名单功法的同名秘籍 ----
    def _item_blocked(self):
        """道具黑名单的归一化名集合。"""
        return item_blacklist_keys(self._item_blacklist)

    def _item_blocked_names(self):
        """道具黑名单原名列表(传给「新增道具」等对话框, 让它们完全不显示)。"""
        return [it.get("name") for it in self._item_blacklist if it.get("name")]

    def _open_item_blacklist(self):
        """打开「道具黑名单…」管理框; 关闭后刷新缓存(候选过滤下一次生效)。"""
        dlg = ItemBlacklistDialog(self, items=self._item_blacklist,
                                  on_purge=self._purge_item_blacklist_ui)
        dlg.exec()
        self._item_blacklist = load_item_blacklist()
        self.statusBar().showMessage(
            "道具黑名单: 共 %d 个 —— 添加道具/一键添加全部/商店上架时都不会再出现它们"
            % len(self._item_blacklist), 6000)

    def _purge_item_blacklist(self):
        """把主角包裹(ItemHad)里已在黑名单的道具删掉; 返回删除个数(只改内存, 点「保存」才写回)。

        无搜索过滤时走局部删行(不重建整表), 有过滤时整表重建(过滤下行号≠数据序)。
        """
        blk = self._item_blocked()
        if not blk or not self._items:
            return 0
        rows = sorted((i for i, it in enumerate(self._items)
                       if _kangfu_norm(it.name) in blk), reverse=True)
        if not rows:
            return 0
        if self._filter_map is None:
    ...
    def _purge_item_blacklist_ui(self):
        """「⚠ 清除包裹里…」: 确认后把主角包裹(ItemHad)里已在黑名单的道具删掉。"""
        if self._text is None or not self._items:
            QMessageBox.information(self, "提示", "请先「打开/粘贴」存档, 再清除包裹里的黑名单道具")
            return
        blk = self._item_blocked()
        if not blk:
            QMessageBox.information(self, "提示", "道具黑名单是空的 —— 先把会卡死的道具/秘籍加进来")
            return
        hits = [it.name for it in self._items if _kangfu_norm(it.name) in blk]
        if not hits:
            QMessageBox.information(
                self, "提示",
                "主角包裹里没有黑名单里的道具(共检查 %d 个)。" % len(self._items))
            return
        shown = "、".join(hits[:12]) + (" 等 %d 个" % len(hits) if len(hits) > 12 else "")
        if QMessageBox.question(
                self, "确认清除",
                "把主角包裹里的 %d 个黑名单道具删掉?\n%s\n\n"
                "(只改内存; 点工具栏「保存 / 另存为 / 复制结果」才写回存档)"
                % (len(hits), shown)) != QMessageBox.StandardButton.Yes:
            return
        n = self._purge_item_blacklist()
        self.statusBar().showMessage(
            "已从包裹里删除 %d 个黑名单道具, 点「保存」写回存档" % n, 8000)

    # ---- 一键清除: 角色身上已学的黑名单功法(v2.16.0) ----
    def _purge_kangfu_blacklist_ui(self):
        """「⚠ 清除已学…」: 选角色范围(全部/主角/某个队友)后把已学的黑名单功法删掉。"""
        if self._text is None:
            QMessageBox.information(self, "提示", "请先「打开/粘贴」存档, 再清除已学的黑名单功法")
            return
        blk = self._kangfu_blocked()
        if not blk:
            QMessageBox.information(self, "提示", "功法黑名单是空的 —— 先把已知会卡死的功法加进来")
            return
        # 角色范围下拉(全部角色 + 主角 + 每个队友)
        choices = ["全部角色(主角+队友)", "主角"]
        scopes = {"全部角色(主角+队友)": "__all__", "主角": tuple(SCOPE_PLAYER)}
        for f in self._char_friends:      # [(group, key, 显示名, scope)]
            label = "队友·%s" % f[2]
            choices.append(label)
            scopes[label] = tuple(f[3])
        pick, ok = QInputDialog.getItem(self, "清除已学的黑名单功法",
                                        "要清除哪些角色身上「已学过」的黑名单功法?", choices, 0, False)
        if not ok or not pick:
            return
        target = scopes.get(pick)
        self._purge_kangfu_blacklist(None if target == "__all__" else target)

    def _char_scopes(self):
        """当前存档里全部角色的 [(标签, scope)]: 主角在最前, 然后每个队友。"""
        out = [("主角", list(SCOPE_PLAYER))]
        for f in self._char_friends:
            out.append(("队友·%s" % f[2], list(f[3])))
        return out

    def _purge_kangfu_blacklist(self, scope=None, confirm=True):
        """把角色身上已学的黑名单功法删掉(scope=None → 全部角色)。返回删除数。

        删除只改内存(self._text), 与其它改动一样点「保存」才写回文件。
        """
        if self._text is None:
            return 0
        blk = self._kangfu_blocked()
        if not blk:
            return 0
        pairs = self._char_scopes()
        if scope is not None:
            scope_t = tuple(scope)
            pairs = [p for p in pairs if tuple(p[1]) == scope_t]
        targets = [(label, self._loc_cached(sc)) for label, sc in pairs]
        text = self._text

        def _work():
            nt, removed = kangfu_purge_blacklisted(text, targets, blk)
            if removed:
                _get_struct_index(nt)
            return nt, removed

        ok, r = self._run_bg(_work, "正在清除已学的黑名单功法…请稍候")
        if not ok:
            self.statusBar().showMessage("清除失败: %s" % r, 5000)
            return 0
        nt, removed = r
        n = sum(len(g) for _l, g in removed)
        if not n:
            QMessageBox.information(
                self, "没有需要清除的",
                "所选角色身上没有已学的黑名单功法(共检查 %d 个角色的功法段)。" % len(pairs))
            return 0
        if confirm:
            lines = "\n".join("%s: %s" % (lab, "、".join(g[:8]) + (" 等 %d 条" % len(g) if len(g) > 8 else ""))
                              for lab, g in removed)
            if QMessageBox.question(
                    self, "确认清除",
                    "将从 %d 个角色身上删除 %d 门黑名单功法:\n%s\n\n"
                    "(只改内存; 点工具栏「保存 / 另存为 / 复制结果」才写回存档)"
                    % (len(removed), n, lines)) != QMessageBox.StandardButton.Yes:
                return 0
            # 用户可能在确认前点了别的…这里直接按最新文本重跑一次最稳(确认框期间文本不会变)
        self._text = nt
        self._mark_dirty()
        for lab, _g in removed:
            for label, sc in pairs:
                if label == lab:
                    key = tuple(sc)
                    ui = self._kangfu_ui.get(key)
                    if ui is not None:
                        ui["dressed"] = None     # 文本已变 → 填充时按新文本重算
                    self._fill_kangfu_list(key)
                    break
        self.statusBar().showMessage(
            "已删除 %d 门已学的黑名单功法(%d 个角色), 点「保存」写回存档" % (n, len(removed)), 8000)
        return n
    ...
    # --- 队友页 ---
    @staticmethod
    def _friend_head_text(group, key, disp):
        """队友页页头文字(带队伍状态后缀)。"""
        gtag = FRIEND_GROUP_LABELS.get(group, group) if group != FRIEND_GROUP_IN else ""
        return ("队友：%s(%s)%s  · 修改后点「保存/另存为/复制结果」写回存档。"
                % (disp, key, ("·%s" % gtag) if gtag else ""))

    @staticmethod
    def _friend_tab_text(group, disp):
        """队友页签文字(离队会带「(离队)」后缀)。"""
        suffix = "" if group == FRIEND_GROUP_IN else "(%s)" % FRIEND_GROUP_LABELS.get(group, group)
        return "队友·%s%s" % (disp, suffix)
    ...
    def _build_friend_page(self, group, key, disp, scalars=None, dicts=None, kangfu=None,
        ...
        page = self._char_page_scroll()
        lay = self._char_inner_lay
        head = QLabel(self._friend_head_text(group, key, disp))
        head.setWordWrap(True)
        head.setStyleSheet("font-weight:bold;")
        lay.addWidget(head)
        lay.addWidget(self._friend_party_box(group, key, disp, scope))
        ...
        ent = self._friend_pages.get(key)
        if ent is not None:      # 登记页控件: 搬移后按 key 反查页签下标(同名队友也能区分)
            ent["head"] = head
            ent["page"] = page
        return page

    # --- 队友 入队 / 离队(v2.17.0) ---
    def _friend_party_box(self, group, key, disp, scope=None):
        """队友页顶部「队伍状态」框: 显示当前在队/离队, 并给「加入队伍 / 改为离队」按钮。

        控件登记进 `self._friend_pages[key]`, 搬移后由 `_party_after_move` **就地刷新**
        (不重建整档角色页 —— 真实 10MB 档 25 页重建要 ~3.8s)。
        """
        in_team = (group == FRIEND_GROUP_IN)
        box = QGroupBox("队伍状态(在队 / 离队)")
        v = QVBoxLayout(box)
        lab = QLabel("当前: <b>%s</b>%s" % (friend_group_label(group),
                                          "" if in_team else " —— 不在队伍里(游戏里不会跟随)"))
        lab.setWordWrap(True)
        v.addWidget(lab)
        h = QHBoxLayout()
        b_in = QPushButton("→ 加入队伍(在队)")
        b_in.setToolTip(
            "把 %s 整条从「离队 LeaveFriends」搬进「在队 AddFriends」。\n"
            "只换容器: 等级/经验/技能/装备/功法/外观等字段原样保留(逐字节不动);\n"
            "搬完点工具栏「保存/另存为/复制结果」才写回存档。" % disp)
        b_in.setEnabled(not in_team)
        b_in.clicked.connect(lambda _=False: self._on_friend_set_group(key, disp, FRIEND_GROUP_IN))
        b_out = QPushButton("→ 改为离队")
        b_out.setToolTip(
            "把 %s 整条从「在队 AddFriends」搬进「离队 LeaveFriends」。\n"
            "若它正在出战队伍(BattleFriend, 实测最多 3 人)里, 会一并从出战队伍摘掉,\n"
            "免得留下「出战名单里的人是离队状态」。" % disp)
        b_out.setEnabled(in_team)
        b_out.clicked.connect(lambda _=False: self._on_friend_set_group(key, disp, FRIEND_GROUP_OUT))
        h.addWidget(b_in)
        h.addWidget(b_out)
        h.addStretch(1)
        tip = QLabel("★ 出战队伍(BattleFriend, 最多 3 人)请在游戏里点选, 这里只改「在队伍 / 离队」归属")
        tip.setStyleSheet("color:#888;")
        tip.setWordWrap(True)
        h.addWidget(tip)
        v.addLayout(h)
        if scope is not None:
            ent = self._friend_pages.setdefault(key, {})
            ent.update({"group": group, "disp": disp, "scope": scope,
                        "state": lab, "b_in": b_in, "b_out": b_out})
        return box

    def _on_friend_set_group(self, key, disp, to_group):
        """单个队友: 加入队伍 / 改为离队(走统一的批量入口)。"""
        if self._text is None:
            QMessageBox.information(self, "提示", "请先「打开/粘贴」含队友数据的存档")
            return
        ok, payload = self._apply_party_moves([(key, to_group)])
        if not ok:
            QMessageBox.information(self, "提示", self._party_skip_msg(payload))
            return

    @staticmethod
    def _party_skip_msg(skipped):
        """把 friends_move 的跳过列表拼成一句提示。"""
        if not skipped:
            return "没有可调整的队友"
        parts = ["%s(%s)" % (k, why) for k, why in skipped[:8]]
        more = "" if len(skipped) <= 8 else " 等 %d 人" % len(skipped)
        return "以下队友未调整:\n%s%s" % ("\n".join(parts), more)

    def _apply_party_moves(self, moves):
        """把队友在 在队/离队 之间搬移(后台线程搬移, 完成后**就地**刷新受影响队友页)。

        返回 (ok, 成功时=已搬移的 [(键, 目标容器)] / 失败时=跳过原因或错误文本)。
        ★ 不做整档角色页重建: 搬移只改队友所属容器, 页内其它数据(属性/功法/装备)完全没变 ——
          重建 25 页要 ~3.8s, 这里只把该页的 scope 路径原地改掉 + 刷页签/页头/按钮(~几十 ms)。
        """
        if self._text is None or not moves:
            return False, [(m[0], "请先打开存档") for m in (moves or [(None, None)])]
        text = self._text

        def _work():
            nt, done, skipped = friends_move(text, moves)
            if done:
                _get_struct_index(nt)   # 预热新文本索引: 下一次操作不再冷建(大档 ~1s)
            return nt, done, skipped

        ok, r = self._run_bg(_work, "正在调整队伍(加入队伍 / 改为离队)…请稍候")
        if not ok:
            return False, [(m[0], r) for m in moves]
        nt, done, skipped = r
        if not done:
            return False, skipped
        self._text = nt
        self._mark_dirty()
        self._party_after_move(done)
        n_in = sum(1 for _k, t in done if t == FRIEND_GROUP_IN)
        msg = "队伍已调整: 加入队伍 %d 人 · 改为离队 %d 人" % (n_in, len(done) - n_in)
        if skipped:
            msg += " · 跳过 %d 人(%s)" % (len(skipped), skipped[0][1])
        self.statusBar().showMessage(msg, 5000)
        return True, done

    def _party_after_move(self, done):
        """搬移成功后**就地**刷新受影响的队友页(不重建角色页)。

        三处联动: ①页内 scope 列表(页面里所有回调都引用同一个 list 对象 → 原地改容器名即可);
        ②按 scope 元组做键的登记表(_char_spin_map / _kangfu_ui / _char_open_cache);
        ③页签文字与页内「队伍状态」框、页头文字。
        """
        if self._text is None or not done:
            return
        opens = {}
        for key, to in done:
            for kd, _ks, vs, _vc in friend_entries(self._text, to):
                if kd == key:
                    opens[key] = vs
                    break
        for key, to in done:
            old_group = FRIEND_GROUP_OUT if to == FRIEND_GROUP_IN else FRIEND_GROUP_IN
            ent = self._friend_pages.get(key) or {}
            disp = ent.get("disp") or key
            scope = ent.get("scope")
            if scope is not None and len(scope) >= 4:
                scope[2] = to          # ★ 原地改: 页内全部回调自动改用新路径
            self._rekey_char_scope(("saveFriendData", "value", old_group, key),
                                   ("saveFriendData", "value", to, key))
            self._char_open_cache[("saveFriendData", "value", to, key)] = \
                (self._text, opens.get(key))
            ent.update({"group": to})
            head = ent.get("head")
            if head is not None:
                head.setText(self._friend_head_text(to, key, disp))
            state = ent.get("state")
            if state is not None:
                state.setText("当前: <b>%s</b>%s" % (
                    friend_group_label(to),
                    "" if to == FRIEND_GROUP_IN else " —— 不在队伍里(游戏里不会跟随)"))
            b_in, b_out = ent.get("b_in"), ent.get("b_out")
            in_team = (to == FRIEND_GROUP_IN)
            if b_in is not None and b_out is not None:
                b_in.setEnabled(not in_team)
                b_out.setEnabled(in_team)
            page = ent.get("page")
            ix = self.tabs.indexOf(page) if page is not None else -1
            if ix >= 0:
                self.tabs.setTabText(ix, self._friend_tab_text(to, disp))
            # 「一键清除已学」的角色范围列表用的也是同一份 scope + 容器名
            for i, (g, kk, nn, sc) in enumerate(self._char_friends):
                if kk == key and g == old_group:
                    if isinstance(sc, list) and len(sc) >= 4:
                        sc[2] = to
                    self._char_friends[i] = (to, kk, nn, sc)

    def _rekey_char_scope(self, okey, nkey):
        """把按 scope 元组做键的登记表从 okey 改到 nkey(队友搬容器后, 页内数据仍然有效)。"""
        for k in [k for k in self._char_spin_map if k[0] == okey]:
            self._char_spin_map[(nkey, k[1])] = self._char_spin_map.pop(k)
        if okey in self._kangfu_ui:
            self._kangfu_ui[nkey] = self._kangfu_ui.pop(okey)
        self._char_open_cache.pop(okey, None)
        self._char_open_cache.pop(nkey, None)

    def _open_party_dialog(self):
        """工具栏「队伍管理…」: 列出全部队友的 在队/离队 状态, 批量切换。"""
        if self._text is None:
            QMessageBox.information(self, "提示", "请先「打开/粘贴」含队友数据的存档")
            return
        recs = friend_records(self._text)
        if not recs:
            QMessageBox.information(self, "提示",
                                    "当前存档里没有队友数据(saveFriendData)。\n"
                                    "请打开含队友的存档(或在游戏里结识队友后再存档)。")
            return
        dlg = PartyDialog(self, recs, self._party_apply_for_dialog, friends_battle_list(self._text))
        dlg.exec()

    def _party_apply_for_dialog(self, moves):
        """PartyDialog 的执行回调: 返回 (ok, 新的队友记录 / 跳过原因文本)。"""
        ok, payload = self._apply_party_moves(moves)
        if not ok:
            return False, self._party_skip_msg(payload)
        return True, friend_records(self._text or "")
    ...
    def _on_add(self):
        ...
        if not dlg.exec():
            return
        raw_names = dlg.chosen_names()
        count = dlg.count_spin.value()
        iblk = self._item_blocked()          # 黑名单兜底: 手动输入/自定义名也可能撞上黑名单
        names = [n for n in raw_names if _kangfu_norm(n) not in iblk]
        n_item_blk = len(raw_names) - len(names)
        ...
        if not names:
            if blocked:
                ...
                QMessageBox.information(
                    self, "提示",
                    "选中的 %d 个道具都在「道具黑名单」里, 已全部跳过。\n"
                    "(已知会卡死的道具/秘籍先避开; 要重新允许请到工具栏「道具黑名单…」移出)" % n_item_blk)
            return
    ...
    def _on_add_all_items(self):
        ...
        if not raw_names:
            ...
            return
        # 一键添加全部只进 ItemHad: 剔除 equip(进 EquipHad)/skillcom(不显示), 并排除道具黑名单
        iblk = self._item_blocked()
        names, blocked_cat = split_names_by_cats(raw_names, ITEM_ADD_CATS)
        names_all = list(names)
        names = [n for n in names if _kangfu_norm(n) not in iblk]
        n_item_blk = len(names_all) - len(names)
        if not names:
            QMessageBox.information(self, "提示",
                                    "道具名库 %d 个里没有可加入 ItemHad 的道具\n"
                                    "(装备/技能合成类已排除, 装备请用「＋ 添加装备…(EquipHad)」;"
                                    "道具黑名单里 %d 个也已跳过)"
                                    % (len(raw_names), n_item_blk))
            return
        existing = {it.name for it in self._items}
        ...
        if n_item_blk:
            tip_cat += "\n(道具黑名单 %d 个已跳过 —— 已知会卡死的道具/秘籍)" % n_item_blk
...
13. <b>功法黑名单 —— 先避开已知会卡死的功法(v2.15.0)</b>: 工具栏「<b>功法黑名单…</b>」打开管理框
    ...
    · 用途: 部分功法一学就让游戏卡死, 先把已知的先列进去避开, 以后有空再逐条确认到底是
      「剧情还没到」还是别的原因 —— 备注里可以记下当时的现象与猜测。<br>
    · <b>⚠ 清除已学…(v2.16.0)</b>: 把角色身上<b>已经学了</b>的黑名单功法从存档里删掉 ——
      弹框先选角色范围(<b>全部角色(主角+队友) / 主角 / 某个队友</b>), 再汇总列出会删掉哪些条目让你确认;
      删除只改内存, 点工具栏「<b>保存 / 另存为 / 复制结果</b>」才写回存档。
      阶段适配: 一键把已中招的角色清干净(主角四段、队友同名带下划线段都支持)。

14. <b>道具黑名单 —— 黑名单功法的同名秘籍(v2.16.0)</b>: 工具栏「<b>道具黑名单…</b>」
    (数据存程序目录 <code>道具黑名单.json</code>, 与存档无关)。<br>
    · 为什么需要: 功法<b>秘籍本身就是道具</b>(在 ItemHad 里), 拿到/添加同名秘籍同样可能触发卡死 ——
      所以把黑名单功法的秘籍一并列进来避开。(实测: 功法名库 1045 门 <b>100%%</b> 都能在道具名库里
      找到同名秘籍, 所以可以直接同名导入)<br>
    · 填入黑名单的道具: <b>「＋ 新增道具…」「一键添加全部道具」「商店 → 添加卖品…」的候选里都不显示</b>,
      也不会被加进主角包裹(手动输入的名字也会被拦下并提示)。<br>
    · 管理框里可以: <b>「＋ 从道具库添加…」</b>(分类树+搜索+多选/全选) /
      <b>「↧ 从功法黑名单导入同名秘籍」</b>(一键把黑名单功法的秘籍全部引进来) /
      <b>「－ 删除选中」</b> / <b>「清空全部」</b> / 逐条写<b>备注</b>; 改动即时写盘。<br>
    · <b>⚠ 清除包裹里…</b>: 把主角包裹(ItemHad)里已在黑名单的道具一键删除
      (同样只改内存, 点保存才写回)。<br>
    · 两者配合的用法: 先把会卡死的功法列入<b>功法黑名单</b> → 点「↧ 从功法黑名单导入同名秘籍」
      把它们的秘籍导入<b>道具黑名单</b> → 再用两边的「⚠ 清除…」把主角/队友身上已中招的功法与秘籍清掉,
      以后就不会再手滑加进来。

15. <b>队友「加入队伍 / 改为离队」(v2.17.0)</b>: 存档把队友分装在两个容器里 ——
    <b>在队 AddFriends</b>(游戏里会跟随你) 与 <b>离队 LeaveFriends</b>。<br>
    · <b>每个队友页顶部新增「队伍状态(在队 / 离队)」框</b>: 显示当前状态, 并给两个按钮
      「<b>→ 加入队伍(在队)</b>」「<b>→ 改为离队</b>」(已是那个状态时按钮自动灰显)。<br>
    · <b>工具栏「队伍管理…」</b>: 一次列出全部队友(名字/英文键/队伍状态/是否在出战名单),
      可 Ctrl/Shift 多选后批量「✓ 加入队伍(在队)」/「✗ 改为离队」(配合「全选」可一次全搬)。<br>
    · <b>是「整条搬移」</b>: 只换容器, 队友对象<b>逐字节不动</b> —— 等级/经验/属性/技能/装备/
      功法/外观/好感……全部原样保留, 游戏里就是同一个人换了队伍状态; 存档其它段落同样逐字节不变
      (实测 20 个真实存档: 出入往返可做到逐字节一致, 且区间外内容完全不动)。<br>
    · <b>出战队伍 BattleFriend(实测最多 3 人, 恒为在队成员的子集)与「在队伍」是两件事</b>:
      改为离队时会<b>顺带把它从出战队伍里摘掉</b>(避免出战名单里留着已离队的人);
      加入队伍时<b>不会</b>自动进出战阵容 —— 出战 3 人请在游戏里点选。想直接改存档里的出战名单,
      可在队伍管理框里看「出战队伍」列确认现状。<br>
    · 搬完点工具栏「<b>保存 / 另存为 / 复制结果</b>」才写回存档; 搬移只改这两个容器,
      <b>不会动</b>角色的其它数据(不会再出现「加了队友属性被重置」的情况)。<br>
    · 页签里「<b>队友·名字(离队)</b>」的后缀、页内状态框会即时更新; 页签位置保持不动
      (重新打开存档时会按文件顺序重排: 在队的在前)。
...
- <b>功法黑名单.json</b>(v2.15.0, 由工具栏「功法黑名单…」维护, 不需要脚本生成): 已知会让游戏卡死的
  ...
  还能「⚠ 清除已学…」把角色身上已学的中招功法删掉; 与存档无关, 换档/重开程序都生效。<br>
- <b>道具黑名单.json</b>(v2.16.0, 由工具栏「道具黑名单…」维护): 已知会出问题的道具(主要是黑名单功法的
  <b>同名秘籍</b>)—— 新增道具/一键添加全部/商店上架 都不会再出现它们, 也不会被加进包裹;
  支持「↧ 从功法黑名单导入同名秘籍」与「⚠ 清除包裹里…」; 与存档无关。<br>
